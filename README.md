...
Implementation is underway using PostgreSQL, Go 1.27.1, and FFmpeg 9.0.1. It now includes authentication, a persistent media catalog, safe file probing, bounded scans, local NFO metadata, persistent genre/tag/studio/person identities, indexed artwork, library-level access control, initial Emby browsing, original-file playback with durable user state, external SRT/WebVTT, client-session capabilities and player-state projection, user-state events and initial remote control, NextUp queries, and React/MUI user/library/task administration. Additional events, embedded/additional subtitle handling, conversion, and hardware decoding/encoding remain in progress. See [build/run instructions](docs/development/running.md), [external subtitles](docs/development/external-subtitles.md), [client sessions](docs/development/client-sessions.md), [WebSocket events](docs/development/websocket-events.md), [NextUp behavior and evidence limits](docs/development/next-up.md), and [implementation progress](docs/development/progress.md).
