...
Implementation is underway using PostgreSQL, Go 1.27.1, and FFmpeg 9.0.1. It now includes authentication, a persistent media catalog, safe file probing, bounded scans, local NFO metadata, persistent genre/tag/studio/person identities, indexed artwork with image transformations and caching, library-level access control, initial Emby browsing, original-file playback with durable user state, and React/MUI user/library/task administration. External subtitles, client-session/events, conversion, and hardware decoding/encoding remain in progress. See [build/run instructions](docs/development/running.md), [original playback](docs/development/direct-playback.md), [local metadata](docs/development/local-metadata.md), [local artwork](docs/development/local-artwork.md), and [implementation progress](docs/development/progress.md).
