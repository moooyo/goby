...
startup window or repeat starts. A new consumer runtime binding remains pending.
...
file and lock are closed. Freeze the final M5 source after combining the prepared
deletion and diagnostic changes and their required interaction test, then run
the affected backend, browser and frontend checks. The consumed catalog input
...
are available for the planned transition after new runtime binding; the catalog
...
| `codex/m5-user-deletion`, generator source `83cf0a8` | [Independent M5 implementation draft](https://github.com/moooyo/goby/blob/83cf0a8/docs/development/native-user-deletion.md): native DELETE, atomic audit and authorization, consumer retirement, administrator UI, migration0029 and version-specific backup/restore tests. The real FFmpeg/pidfd/WebSocket regression and frontend capture tooling are prepared. The [real schema-29 export](../development/m5-user-deletion-catalog-generation-20260916.json) was generated once from its complete 5363-file source, reviewed and returned byte-for-byte; PG and all temporary resources are closed | Combine the prepared deletion and diagnostics changes with the required cancellation/reservation/peer-isolation test, freeze that final source, then run the prepared Go/DB/HTTP/browser and final verification. Preserve the actual frontend graph and successful build result together. The generator input is consumed; no M5 acceptance or merge into frozen Programs is claimed |
...
2. Reuse the reviewed September 16 application recovery, separate post-start
   ...
   Prepare the new consumer runtime authority and bind it to the actual verified
   Programs artifact through the existing components before the one direct A
