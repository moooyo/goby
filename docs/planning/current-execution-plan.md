...
the two CLI sources are published byte-for-byte. The fresh capture input and
retention derivation are prepared; an actual capture result is not yet recorded.
...
is now published and its final 142 component checks passed. Actual capture/product
loader verification and transition admission remain pending.
...
| --- | --- | --- |
| `codex/programs-theme-diagnosis`, `3d6b79b`; binding tooling integrated as `f45d73c` | [Complete-source regression/build](../development/programs-complete-source-full-result-20260916.md) passed all 25 packages and both builds. The product receipt, `db22e3d...` runtime envelope and byte-identical CLI copies are published; [44/28/70 final binding checks](../development/programs-final-binding-components-20260916.json) passed once with independent closure review | The fresh capture input/retention derivation await an actual result. Complete actual product/runtime loader and recovery admission before transition; post-transition v4 and client work remain pending. The old 138 results and declared mount-profile gap remain unchanged |
...
2. Reuse the reviewed September 16 application recovery, separate post-start
   ...
   checks and byte-identical published CLI files. The prepared capture/retention
   inputs still need an actual result, followed by product/runtime loader and
   recovery admission joining that authority to the artifact before the one direct A
