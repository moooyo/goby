...
| `codex/m6-frontend-contributions`, `808edc2` | [Private frontend contribution capture draft](https://github.com/moooyo/goby/blob/808edc289ab18d132d0d559fbfbb2b327188a3e4/docs/development/frontend-contribution-capture.md): Vite records module/source/package and actual asset hashes outside `dist`; release accepts an explicit report, verifies the full asset set and preserves a private copy. Six top-level Node cases and eight subcases are authored but unexecuted. Static review corrections address incomplete package identities, report-parent replacement and final source/copy readback | At the next independently required frontend build, run Node/type checks and actual callback/asset/consumer verification in the coordinated window. Include `web/admin/build/` in the source snapshot and retain the successful command receipt with the sidecar. No unchanged E11 rebuild, Programs artifact change, build success or legal completeness is claimed |
