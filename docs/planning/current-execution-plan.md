...
| 2. Native scan and concurrent HTTP capacity — implementation and verification in progress | Complete and remotely verify the integrated controller for the [fixed 1,000-file native measurement](../development/native-scan-http-capacity-plan.md), then run its two ordinary scans and bounded independent readers | [Recorded preparation](../development/native-scan-http-capacity-preparation.json) includes current-environment observations and twelve passing synthetic reader groups after a preserved first failure. No native execution is admitted. Require actual source-bound HTTP/SQL measurements and complete owned closure; do not infer representative throughput, invent an SLO, or reuse the paused M2 mount |
