...
Current priority: **verify the combined M5 increment and bind the verified Programs artifact to recovered runtime authority**.
...
scope `/opt/goby-test/m5-combined-full-fcf3c8f4a474` is running and actual Go has
started. It retains the exclusive verification window; final outcomes remain pending.
...
at `a5af0b6`. Publish the source revision covered by these 151 checks in a fresh execution scope and
prepare capture02; no new live capture follows from the component result. Preserve
the consumed first scope and the separate 142 evidence.
Actual product/runtime loaders and post-transition v4 records remain unverified.
The old 138 results, transition admission and hold remain unchanged. Query
...
passed. Fresh source publication/capture02 preparation, actual product loading
and transition admission remain pending.
...
scope is now running with actual Go started. Await its result and closure;
the prior failure and storage acceptance remain unchanged.
...
| --- | --- | --- |
| `codex/programs-theme-diagnosis`, `3d6b79b`; latest metadata tooling integrated as `a5af0b6` | [Complete-source regression/build](../development/programs-complete-source-full-result-20260916.md) passed all 25 packages and both builds. The product receipt and `db22e3d...` envelope are published; the original 142 components and later [151 metadata checks](../development/programs-generation-key-metadata-verification-20260916.md) passed in separate scopes. Capture01 failed and independently closed | Publish the source revision covered by the 151 checks in a fresh scope and prepare capture02. No successful live capture or actual product-loader acceptance exists yet; recovery/transition, post-transition v4 and client work remain pending. Preserve capture01, original 138/142 evidence and the declared mount-profile gap |
| `codex/m5-deletion-diagnostics`, `138522b` | [Combined M5 source](../development/m5-combined-source-preparation-20260916.json) includes the real schema-29 export, post-DELETE cancellation hook and reservation/peer-isolation regression. The real frontend and storage prerequisite passed. The old `195824` full attempt failed capacity and closed; fresh scope `m5-combined-full-fcf3c8f4a474` is running with actual Go started | Await the active full result and closure before further verification. Require `--frontend-contributions` for the final package. Preserve the old capacity failure, both failed storage scopes and consumed catalog input. No final build or M5 acceptance is claimed |
...
2. Reuse the reviewed September 16 application recovery, separate post-start
   ...
   and independently closed. Its bounded key metadata-rule correction now passed
   151 checks and independent review. Publish the fresh verified sources and
   prepare capture02, followed by a complete reviewed capture and product/runtime
   loader and recovery admission joining that authority to the artifact before the one direct A
   transition. The separate schema-29 catalog job has already closed; its real
