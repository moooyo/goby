...
Keep that scope consumed and prepare a fresh one when capacity is sufficient.
...
SSH numeric exit remains unknown/null. Verify the separately prepared key metadata
correction, then use a new reviewed scope/input; the first scope is consumed.
...
capture failed and closed; its correction/new scope, actual product loader and
transition admission remain pending.
...
scope or relax its gate. A fresh capacity-admitted scope is still needed for Go
and both builds; the storage acceptance remains unchanged.
...
| --- | --- | --- |
| `codex/programs-theme-diagnosis`, `3d6b79b`; binding tooling integrated as `f45d73c` | [Complete-source regression/build](../development/programs-complete-source-full-result-20260916.md) passed all 25 packages and both builds. The product receipt, `db22e3d...` runtime envelope and CLI copies are published; 44/28/70 components passed. The [first fresh capture](../development/programs-state-capture-first-attempt-20260916.md) failed at the key-path guard and independently closed | Verify the bounded key metadata-rule correction and prepare a new capture scope. No reviewed state or actual product-loader acceptance exists yet; recovery/transition, post-transition v4 and client work remain pending. Preserve the consumed failure, original 138/142 sources/results and declared mount-profile gap |
| `codex/m5-deletion-diagnostics`, `138522b` | [Combined M5 source](../development/m5-combined-source-preparation-20260916.json) includes the real schema-29 export, post-DELETE cancellation hook and reservation/peer-isolation regression. The real frontend and storage prerequisite passed. The [actual full attempt](../development/m5-combined-full-capacity-failure-20260916.md) failed its internal capacity gate and independently closed before worker creation | Use a fresh scope/input when the unchanged 4 GiB gate is met, then run Go/DB/HTTP/browser and final builds against complete S1. Require `--frontend-contributions`. Preserve the capacity failure, both failed storage scopes and consumed catalog input. No Go/build result, M5 acceptance or merge into frozen Programs is claimed |
...
2. Reuse the reviewed September 16 application recovery, separate post-start
   ...
   and independently closed. Verify its bounded key metadata-rule correction,
   then use a new scope for a complete reviewed capture, followed by product/runtime
