...
| M6 / packaged service / OCI | Linux amd64 software recipe and runtime configuration are prepared on `codex/m6-oci-package` at `18cd4ef`; image, runtime and upgrade acceptance remain open. | [Prepared profile](https://github.com/moooyo/goby/blob/18cd4efd117f3314cf5cb46b4736f65795bbdf59/deploy/oci/README.md), [source pins](https://github.com/moooyo/goby/blob/18cd4efd117f3314cf5cb46b4736f65795bbdf59/deploy/oci/source-pins.json) | In the coordinated window, select a verified release, establish the owned builder/runtime/storage boundary, and validate the image plus nonroot software, backup/restore and stop/start behavior. | Available compatible engine, allocated resources, PostgreSQL/TLS role and actual private mounts; other architecture/device profiles retain their prerequisites. | Later deployment profile; required for OCI support. Source recipe and registry metadata do not establish an image or runtime pass |
...
| M6 / actual package contents and external distribution / each selected package | Initial notices, emitted chunk graph, 73-row fixed-commit MUI correspondence and 19-text payload draft recorded; project license, remaining provenance/contribution and final legal payload open. | [Component inventory](../development/systemd-package-component-inventory.md), [MUI source lookup](../development/systemd-package-mui-source-correspondence.md), [payload draft](../development/systemd-package-legal-payload-draft.md) | Capture source contributions with the next required frontend build; resolve relevant upstream material and assemble the payload for the actual selected package. Preserve E11 historical gaps and the pending project-license decision. | User's pending license choice and required upstream information; this matrix makes no legal conclusion. | External distribution gate; no unchanged E11 rebuild, repeated license question or external release implied |
