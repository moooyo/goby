...
The current retained runtime corpus has 958 records. The latest [metadata reference study](metadata-reference.md) adds 130 records, including 106 complete HTTP exchanges, and separates sorting, locks, NFO import, forced refresh and reset. The earlier [video study](video-progressive-reference.md) preserves incomplete responses and does not establish general copied-video seeking. Current implementation and acceptance results are tracked separately in [implementation progress](../development/progress.md).
...
The implemented [native metadata contract](../api/admin-metadata.md) now provides these durable administrator layers for local metadata, with library item management, revision checks, explicit restoration of automatic values and inactive-setting cleanup. Its [Linux/browser verification](../development/verification-m5b-metadata.md) is separate from the [reference mutation study](metadata-reference.md); the latter shows that Emby's forced-refresh and sorting behavior cannot be inferred from the SDK field names alone. Online providers and generic task scheduling remain additional work.
