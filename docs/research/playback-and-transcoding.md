...
The official [WebSocket guide](https://dev.emby.media/doc/restapi/Web-Socket.html) specifies changing the server base scheme from `http`/`https` to `ws`/`wss`, with query `api_key` and `deviceId`. The subsequently pinned official SDKs append `/embywebsocket`, and the [M3c reference investigation](websocket-reference.md) recorded accepted upgrades on five paths. Valid-token event delivery was verified on `/embywebsocket`; an accepted upgrade alone did not establish authentication. Keepalive duration, reconnection, and equivalent event delivery on every alias still need broader coverage.

Messages use `MessageType` and `Data`; the reference also included `MessageId` on UserDataChanged. The current guide describes UserDataChanged, Play, Playstate, and GeneralCommand delivery, while the pinned JavaScript SDK posts progress over HTTP. In controlled paused playback, incoming ReportPlaybackProgress text/binary frames did not apply the position, but the same HTTP body did. Use the verified HTTP report path; neither undocumented incoming message names nor universal lack of support can be inferred from that bounded probe.
