...
[Native Devices](devices.md) lists ordinary registry generations independently of native administrator sessions and application-key contexts. IDs and revisions are positive decimal strings. Device POST bodies are strict 4 KiB JSON; custom names and literal search are bounded to 256 UTF-8 bytes. Manual name edits use revisions that routine activity preserves, and clearing restores each session's raw reported name. `ActiveLoginCount` counts currently eligible stored credentials under the supported account, device and access-schedule policies. It is not online presence or a full current network/media authorization decision. Repeat deletion of a known generation returns its original timestamp and zero newly revoked credentials; a later login registers a new generation. The hidden shared-key family is not exposed through native device routes. Device handlers disable caching and never expose credentials or ciphertext.
...
The [configuration contract](configuration.md) defines closed supported projections. Current source adds desired HTTP port, selected encoding controls, and preferred metadata language/country while retaining four persisted name modes and shared native revisions. Unsupported fields are rejected atomically. Compatibility writes require Emby token authority; native administrator cookies and CSRF tokens remain a separate interface. Other configuration fields and named sections remain unimplemented.
...
| `GET /emby/System/Info` | Emby login or application key; stable identity plus backed capability flags. Managers additionally receive pending-restart state and the observed active HTTP port when a listener is reserved. PublicURL remains deployment-owned; no self-restart, self-update or in-process HTTPS is claimed |
...
| `GET /emby/System/Configuration` | Administrator/key: read-only `IsStartupWizardCompleted`, configured `ServerName`, metadata language/country, `EnableInternetProviders` and desired `HttpServerPortNumber`; ordinary viewer: exact `200 {}`; never the full upstream object |
...
| `GET /emby/Users/Query` | Administrator or application key; current-policy `IsHidden`/`IsDisabled`, literal normalized name bound, deterministic `SortOrder`, filtered count and `StartIndex`/`Limit`; see [user queries](user-query.md) |
...
The query adapter supports `ParentId`, `Recursive`, `SearchTerm`, `StartIndex`, `Limit`, `Ids`, `IncludeItemTypes`, `MediaTypes`, and up to eight supported `SortBy` keys with deterministic ties. The [discovery query contract](discovery-queries.md) lists the complete admitted sort and selected predicate matrix. Optional `IsFolder`, `IsSpecialSeason`, `IsSpecialEpisode` and `IsStandaloneSpecial` filters use actual typed catalog facts before counting or paging; source-backed placement and the upstream interpretation boundary are documented in [TV query filters](../development/client-tv-query-filters.md). User-state filters include `IsPlayed`, `IsFavorite`, and the documented `Filters` subset. Entity filters include `Genres`, `Tags`, `Studios`, `Person`, `GenreIds`, `TagIds`, `StudioIds`, `PersonIds`, and `PersonTypes`. Name lists use a pipe delimiter; IDs accept pipes or commas. Same-dimension values use OR, separate dimensions use AND, and person/type conditions match the same credit association. [Catalog projection](catalog-projection.md) defines field-query casing, exclusions and the UI language hint; [item capabilities](item-capabilities.md) defines actor-bound `CanDelete` and `CanDownload`. Source-backed `Status`, `EndDate` and episode placement follow Fields/detail projection. These implementation contracts do not claim unexecuted phase acceptance or every upstream filter/field.
