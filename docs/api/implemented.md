...
There is no consumer web player by design. Stream endpoints, the HLS job controller and hardware playback are still pending in the [active delivery plan](../planning/delivery-and-verification.md). Richer catalog/media types, generated or embedded artwork, and the complete query/policy surface remain work.
