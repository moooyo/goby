...
The reference inventory contains 610 sanitized JSON records: 436 earlier records plus the 174 records from the [M4c audio study](../research/audio-reference.md), of which 117 are HTTP responses. Record totals include supporting observations and media probes; they are not counts of distinct HTTP operations.
...
| `GET`, `POST /emby/Items/{Id}/PlaybackInfo` | Authorized source facts, original-file and configured HLS negotiation; supported DeviceProfile constraints, copy restrictions and current user conversion policy; progressive-audio DeviceProfile negotiation remains incomplete |
...
Universal and converted audio can accept a fresh client-generated `PlaySessionId` without a preceding PlaybackInfo request. Migration `0012` binds each client nonce under `(user, authentication session, device, client nonce)` to one canonical `play_...` identity and item/media source. Reports, Ping and cleanup resolve that owned binding; HLS revisions, child URLs and encoding jobs use canonical identities. Reusing a nonce cannot change its resource or revive a stopped or expired play, and another authentication session cannot take over the reference. Preparation and media access do not substitute for playback-progress reports. See [client playback references](../development/client-playback-references.md) for lifecycle, tombstones and limits.

Probe cache version 3 retains measured integer audio sample counts and effective packet boundaries for supported continuous audio. Normal library rescans are required after upgrading: older probe snapshots block indexed media and external-subtitle delivery until refreshed. A current snapshot with unproven exact audio timing can still permit original-file playback, while conversion requires proven complete selected-track coverage. Integer samples carry through seeking and resampling without reconstructing them from rounded ticks. WAV uses an accurate prewritten RIFF header and bounded PCM output; substantially short or excessive output fails. Audio HLS merges an encoded tail of at most one output frame into the preceding segment and uses measured packet facts for copy, preserving total duration and stable numbering without advertising an unproducible tail URL. The [audio guide](../development/audio-playback.md) documents the sample quantization boundary, RIFF size limit and remaining timing profiles.
