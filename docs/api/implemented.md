...
Reference total configuration returns administrator `200` with 60 fields and ordinary-viewer `200` with exactly `{}`. Named encoding/devices/DLNA reads return administrator `200`, viewer `403`, and anonymous `401`; an unknown name returns administrator `500`. The earlier fresh mutation study observes a partial update returning `500` after a visible in-process name change, and three key-authorized baseline no-op writes returning `204`. These observations do not establish restart persistence or changed-value key writes. M5h's five [ConfigurationService operations](configuration.md) originally implemented `ServerName`, read-only `IsStartupWizardCompleted`, and `encoding.TranscodingMaxWidth`. Current source additionally supports `PreferredMetadataLanguage` and `MetadataCountryCode`; the full 60/17-field reference objects remain unsupported. Native settings retains the four name modes and independent Encoding state, and adds the closed `Management.Metadata`, `Management.Subtitles`, and `Management.Tasks` sections described below.
...
configuration-write API. See [projection](../../internal/server/user_configuration_dto.go#L34).
...
| `GET /emby/System/Configuration` | Administrator/key: read-only `IsStartupWizardCompleted`, configured `ServerName`, `PreferredMetadataLanguage`, and `MetadataCountryCode`; ordinary viewer: exact `200 {}`; never the full upstream object |
...
| `POST /emby/System/Configuration` | Administrator/key; supported server-name replacement and metadata language/country updates; omitted language/country reset to `en`/`US`, absent/null name selects unset; optional read-only setup-state echo; empty `204`, atomic validation |
...
| `GET`, `HEAD /emby/Videos/{Id}/hls2/{PlaylistId}/{Artifact}`; Audio equivalent | Authorized generated media playlists, initialization/media segments, and selected `subtitles.m3u8`/`subtitles.vtt`; only artifacts allowed by the negotiated plan |
| `POST /emby/LiveStreams/Open`, `/MediaInfo`, `/Close`; `GET`, `HEAD /emby/LiveStreams/{LiveStreamId}/hls/{Artifact}` | Configured HTTP(S) dynamic-source leases and bounded nonseekable TS/fMP4 HLS; source ownership, current policy and cleanup enforced; no Live TV business subsystem |
...
The six [ScheduledTaskService operations](tasks.md) reuse current administrator/key authorization and the same persisted definition ID as native tasks. The supported compatibility key remains `RefreshLibrary`; the new native metadata/subtitle/cache executors do not create additional Emby task keys. `LastExecutionResult.Id` remains the definition ID, while native history uses distinct run IDs. Pending/running projects as `Running`, stopping as `Cancelling`, and no active run as `Idle`; progress counts terminal library children. Compatibility tick fields are JSON numbers, with strict bounded arrays and no partial replacement on invalid input. `SystemEventTrigger` is rejected because no event executor exists. Complete scheduling/client parity and reference timer behavior remain outside this implementation boundary.
...
[Progressive video PlaybackInfo](../development/progressive-video-playback.md) preserves HTTP/HLS profile order and treats omitted/empty Protocol and Context as HTTP/Streaming. Its closed progressive output is MP4/H.264 with AAC for existing audio. The standard `/Videos/{Id}/stream.mp4` URL carries actual source stream indexes, concrete copy/encode settings, and a source-relative start. Compatible zero-start copy and mixed output remain supported. Nonzero video-copy seeks now require an exact indexed H.264 IDR boundary without decode pre-roll, a verified source clock and runtime packet-copy evidence; selected audio must be encoded AAC or absent. Unproven copy requests fail or a normal codec request chooses permitted encoding. Encoded seeks retain the verified format clock and existing software restart/linear fallback paths; deinterlacing retains the required linear history. Software HDR-to-SDR and deinterlacing are also implemented for supported source facts. Exact output targets remain independent of ceilings; URL fields cannot inject a clock origin, duration, filter, or hardware configuration. See [advanced-media limits](../development/advanced-media.md) for the narrow copy/processing contract; generic client-independent copy seeking is not claimed.
