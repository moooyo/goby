...
| `GET`, `POST /emby/Items/{Id}/PlaybackInfo` | Authorized source facts, original-file and configured HLS negotiation; supported DeviceProfile constraints, copy restrictions and current user conversion policy |
...
There is no consumer web player by design. Original-file, external SRT/WebVTT and [HLS VOD](../development/hls-playback.md) endpoints are implemented. Broader subtitle handling, progressive/universal audio, additional conversion profiles, actual hardware playback and complete client acceptance remain open in the [active delivery plan](../planning/delivery-and-verification.md). Richer catalog/media types, generated or embedded artwork, and the complete query/policy surface remain work.

The [conversion engine](../development/transcode-engine.md) supplies PostgreSQL job records, FFmpeg execution and cache supervision. The HLS adapter applies current policy, creates source-global timelines, addresses immutable output revisions and cancels owned work. Configuration and exact supported limits are documented separately; engine/HTTP tests do not establish compatibility with every third-party client or actual GPU execution.
