...
The [M5j native backup/recovery increment](development/backup-recovery.md) is
complete and deployed: encrypted backups, durable operations, administrator UI,
offline CLI, generation activation and rollback. The [final full suite](development/m5j-final-full-race.json)
passed 1605 race tests across 24 packages. UI a6 passed 27 mocked-API cases;
[actual runtime acceptance](development/m5j-runtime-acceptance.json) passed the
browser, restart and offline recovery journeys. [Protected deployment](development/m5j-deployment-evidence.json)
and the [main backup/download/logout workflow](development/m5j-deployed-backup-workflow.json)
passed. That historical deployment used M5j/schema23/probe6, PID **3750313**, with
57 current assets. The initial deployment parser failure and its read-only
finalization retain separate evidence. This is a native Goby feature, not
Emby BackupRestore/plugin compatibility. This historical increment is closed;
the [handoff](development/handoff.md) lists remaining work.
...
| [Machine-readable inventory](api/inventory.json) | Immutable upstream research metadata, source references, and initial planning classification |
...
| [Native backup and recovery](development/backup-recovery.md) | Accepted M5j encrypted archive, verified restoration, offline CLI, generation switching, deployment and evidence |
| [Backup API](api/backups.md) | Administrator cookie/CSRF routes, typed projections, idempotency, restore plans, downloads and rollback |
| [Development handoff](development/handoff.md) | Current closeout record, environment constraints, accepted baseline and next-round priorities |
| [Toolchain and hardware policy](development/toolchain.md) | Stable release pins, official sources, PostgreSQL integration, remote-only verification policy, and decode/encode evidence |
...
- **Goby design:** a Goby-defined contract or implementation decision, including `/admin/v1` APIs; implementation and acceptance status are tracked separately.
