...
The official reference corpus contains **1665 records**. The [key-device study](research/key-devices-reference.md) adds 189 records to the preceding 1476: 186 complete HTTP exchanges, two audits, and one initial connection-refused observation without an HTTP response. The [ordinary user-device study](research/devices-reference.md) and earlier [application-key management](research/api-key-reference.md), [playback](research/api-key-playback-reference.md), [client-context](research/api-key-context-reference.md), and [target-scope](research/api-key-scope-reference.md) evidence remain intact. Records include supporting probes and preserved incomplete responses; they are not counts of implemented endpoints or successful client workflows. Deployed schema **18**, through `0018_application_key_devices.sql`, adds ordinary and shared application-key device generations while preserving earlier credentials and playback history. Probe cache version **6** remains unchanged; device migration requires no media rescan.
