...
Goby serves audio through the Emby Universal and legacy stream routes, alongside
the existing original-file and MPEG-TS HLS delivery paths. The website remains an
administrator dashboard. The [reference study](../research/audio-reference.md)
separates verified Emby 4.9.5.0 behavior from Goby's implementation choices.
...
`MaxAudioChannels` and `MaxSampleRate` ceilings. A valid but unconstructible output
...
ADTS merely because its codec name is AAC. Nonzero FLAC copy starts and other
unproven copy combinations are declined; permitted encoding can supply a valid
alternative. Native WAV copy validates the input WAVE format and data layout.

FLAC preserves known source precision through a 16- or 24-bit output choice;
24-bit sources are not silently reduced to 16-bit. An unknown source precision
...
Probe cache version **3** adds bounded audio packet/frame scanning. For supported
continuous audio, decoded sample counts account for decoder-applied priming,
discard and edit semantics. This fixes raw ADTS duration estimates that would
otherwise truncate an encoded output. The scan also records source-relative
effective packet boundaries used by copied-audio HLS.
...
current verified timing set includes MP3, AAC/ADTS, native FLAC/WAV and supported
audio MP4; additional formats and timing cases remain work.
...
libraries after upgrading to probe version 3; previously cached probe facts do
not establish the new duration contract. Existing local metadata, artwork,
