...
The subsequent [frontend capture draft](https://github.com/moooyo/goby/blob/43a9b76a93615b7f872f48dae4e9b3a73ca050b0/docs/development/frontend-contribution-capture.md)
is committed and pushed as `43a9b76` on `codex/m6-frontend-contributions`, in
...
corrections. Public/emitted JavaScript without a chunk map is now explicit;
the consumer rejects an incomplete or overlapping filename partition. The new
synthetic CLI cases require specific mutation guards and no early Go dispatch
on invalid inputs. Node tests, type checking, actual Vite/build hooks, capture overhead
...
token has logout204/same-token401 closure, and all 158 protected roots and the
complete Goby state remain unchanged. This releases a baseline for a new
preparation; it does not release a matrix fixture or establish client acceptance.

The [observer03 record](nextup-global-baseline-observer-03.md) and its
[independent terminal](nextup-global-baseline-observer-03-independent-terminal.json),
SHA-256 `f5b2521936e18b00787c4d8ec068410d6d142363f74dfba9051577016005bba1`,
bind the actual 62 normal and two cleanup requests. The 61 snapshot GETs retain
the old twelve full details and collect both preparation04 actors' six Episodes
each under the actual administrator token. Older own-token DTOs constrain item
identity and zero UserData; they were not spliced into the fresh full DTOs. The
single recovered P4/A1 projection, P authentication dates and recovery-created
device are reconciled against the actual 172-request parent ledger and separate
six-request recovery. All 207 scope entries, including 204 files, agree with
the complete replay. The worker's original awaiting_independent_attestation
status and baselineReleased=false remain unchanged.

| Current accepted observer input or output | SHA-256 |
| --- | --- |
| Dedicated observer source | `126d625a9b68871b88a78158584d0fb65720fd605436edf288164336625efbc6` |
| Observer guards | `b22478e59006e2d9c12af6e69948e5bbbbc16d43fb62d6c66b722e5e883e0a28` |
| Actual input | `cc59138eda23e99832263d3d85feda89f92a39fda69e7c4146a0609e8ee4a320` |
| Frozen plan | `60ea6d44c9389239370ad56ac5885b70126d29fd0804a52fe76a4519e174a309` |
| Complete private public-baseline.json | `518bf101500b94c1b1c07c2c90934f96ee741265b5058e16a4103b4d45439a0f` |
| Actual closed-authentication.json | `ea9b9311e3c6b7a30270e37c429a725b6a3f6aea86d6a892274aed7c6456e736` |

Input, execution and output roots are respectively
`W/nextup-global-reference-baseline-inputs-03`,
`W/reference-nextup-global-baseline-execution-03`, and
`W/reference-nextup-global-baseline-observation-03`. The observer source is
frozen under `W/nextup-preparation04-baseline-observer-tool-01/revision-02`.
Its [final verification](nextup-preparation04-baseline-observer-verification-02.json)
passed 101 remote guards and two compile checks. The initial revision's 96
passing guards remain retained separately; review subsequently tightened actual
output-file and complete observe-argv bindings before any business HTTP.

The actual unit `goby-nextup-preparation04-baseline-observer-03.service` retains
invocation `475e6e5130ca4ab5947f37bafc60d535`, exit0/MainPID0, absent former
PID1515339 and an empty cgroup. Before preservation SHA-256 is
`77baa27e16bf527e5b676ff497e4bd6b03c626c9cda924312048e0769570e98f`;
after is `972e3a9bdff3c4580da0784cf6057e835fe1a3e13b96df83e424093094428d3b`.
All 158 roots, old service identities, main files and the full Goby state at
83 sessions, 70 devices and 185 activity entries match. This observer scope is
consumed; do not run it again or change its original files.

The [shared playback contract](nextup-playback-percentage-contract.md) remains
verified and published as `d47a733`: matrix53, transport100, preparation161 and
operator65 guards, eight compile checks, and three actual retained Episode DTO
checks. It validates exact finite partial percentages, retains all UserData
values/types/presence, and requires 34 distinct response receipts for the four
calibrations. Each stopped lifecycle permits one DELETE; an unacknowledged
...
source32/schema27 and product publication authority
`16d75c38064008680fa60839c637efee2f12f2ae` are unchanged. Primary upgrade needs
the positive client gate and a fresh scope/input/prepare with independent
upgrade attestation. Full M2-M6 remain open; M7 remains deferred. Actual GPU
verification still requires a remote environment with usable GPU hardware.
All verification used `ssh test-env`; no local verification was performed.
...
The latest source55 original-client v7 tools are frozen and passed 952 guards,
13 real-browser owned-HTML cases and their preflight/history checks. The actual
run failed after discovery/armed, validated native login200 and forward
PUT200/readback. The browser reported `automatic_http_not_observed_within_window`.
The independent terminal confirms native restoration of the original sparse and
effective metadata, two closed sessions, one device, six audits and revision
...
the window despite page-frame receipt of the matching event. Reference UI v3
passed 104 pure guards (49 Python, 27 driver, 28 runtime), 13 isolated DOM cases
and two compilation checks; its tools were published as
`7b07a468811352514c21d60b5683f4268f14e8c1`. Its actual preflight passed with
65 HTTP requests, two authentication POSTs and zero metadata POSTs. Actual v3
then passed viewer login, discovery and armed stages and sent one forward
metadata POST with HTTP 204. Unmodeled `MediaSources[0].Name` derivation and a
mixed administrator/viewer credential baseline stopped complete observation.
The original failed report still says `restoration_required`; a separate
[recovery terminal](reference-library-changed-ui-v3-recovery-terminal.json)
confirms the full target restored except Etag, including the media-source name,
with anchor/media preserved. Both original sessions and the recovery administrator
have HTTP 204/exact-token401 closure evidence. The
[v3 terminal](reference-library-changed-ui-v3-terminal.json) records
`failed_scope_sealed_after_independent_restoration`; 74 history/tool roots and
the full post-v7 Goby state at 83/70/185 are preserved. No complete Name-only
automatic-refresh result was obtained. V4's derived-name contract and actual
same-UI-token baseline have four frozen, remotely verified changed sources.
Reference v4 subsequently completed both 120-second observation windows,
restoration and independent cleanup. Both windows retain the original title and
contain zero automatic HTTP despite one matched LibraryChanged event in each
observation channel. The restoration title matches the expected baseline without a
proven transition. This complete negative control does not satisfy the positive
client gate or establish a Goby-specific protocol defect. V1/v2 history and v2
dispatch uncertainty are unchanged. The public automatic-refresh triggering conditions remain unresolved.
...
The eight frozen v3 client tools and ten safe evidence files were then pushed as
`01bd683a9ea32ce5d6ff8acd02e432883a55fc15`, also without changing product authority.
The eight frozen v4 tools and eight safe evidence files were published as
`f9ab33d0d125aeb5f16aa5a6071da6b8bdd126f9`. That 16-file commit excludes the
independent v4 terminal; product authority remains `16d75c3...`.
V5's eight frozen tools and nine safe records were published in the 17-file
commit `e96e771166ab5afd68639dc3836632838bb6dfd9`. Its controller guard is the
separately verified 05b version; the production controller and other runtime
...
remains `16d75c3...`.
V7's eight changed sources and six safe records were published as
`6eca52537d104e0108b97218339b2b4701f15c60` (14 files). The ninth frozen source,
the DOM guard, was byte-identical and is not a changed file. Product authority
remains `16d75c3...`.
...
one device and four audits. The sealed pre-v7 baseline was 81 sessions,
69 devices and 179 audits. The independently verified current counts are
**83 sessions, 70 devices and 185 audits**, reflecting v7's two sessions, one
device, six audits and two metadata revisions. The actual v7 after-state is
`/opt/goby-test/exec-work-m3e/client-library-changed-ui-source55-v7/after-full.json`,
...
Those two failed client scopes are sealed and consumed, without client acceptance.

TOOL03/v3 subsequently passed [449 loader / 58 driver guards](client-library-changed-source55-v3-guards.json),
...
[two actual prior-ledger checks](client-library-changed-source55-v3-prior-authority.json),
[fresh Python preflight](client-library-changed-source55-v3-preflight.json) and
[six JavaScript real-document checks](client-library-changed-source55-v3-real-documents.json).
The [actual v3 run](client-library-changed-source55-v3.json) still failed with
`library_changed_target_card_not_observed`. Its
[discovery evidence](client-library-changed-source55-v3-discovery.json) pairs
the complete physical exchanges with browser frames: CollectionFolder returned
200 with the exact `Subviews` array, and Items returned 200 with the unique
expected target Id, Name and `Type: Movie`. The page reached the Movies
`videos` route, but `[data-id=target]` matched zero nodes. Duplicate target
siblings and a mismatched recorded item type do not explain this observation.
At the v3 boundary, those records could not establish whether a target card was
visually rendered. The screenshot was omitted with `visible_content_not_safe`,
without identifying the triggering filter. V4 subsequently resolved the
visibility and matcher question below.

The [independent v3 terminal](client-library-changed-source55-v3-terminal.json),
SHA-256 `e53e9777337c8c0b33a0cc86dad2e22c79d5e7f2601aa64dd68c6ed91cd9ecf1`,
records `failed_scope_sealed` and no client acceptance. It confirms this run's
one session, one device, two audits and zero metadata revision delta, with B
closed and no native operation. V1/v2 and their tool/scope files, candidate,
primary, media and all old rows/sequences/private state remain preserved.
No additional cleanup was required. V3 is sealed and consumed and must not be replayed.

V4 passed [513 loader / 61 driver / 127 actor guards](client-library-changed-source55-v4-guards.json),
...
[six real-document checks](client-library-changed-source55-v4-real-documents.json).
The [actual v4 run](client-library-changed-source55-v4.json) retained fixed history
`[2, 3]` and added bounded diagnostics. Its
[DOM and wire evidence](client-library-changed-source55-v4-discovery.json) and
[safe screenshot](client-library-changed-source55-v4-discovery.png) prove that
the Movies page normally displays the unique `M3e Client Movie` card. The card
has no `data-id`; requiring that attribute caused the tool's discovery failure.
The [browser report](client-library-changed-source55-v4-browser.json) remains
failed, and no native metadata edit or LibraryChanged observation ran. Its
ledger is +1 session/+1 device/+2 audits and revision delta0, with B closed.
The [independent v4 terminal](client-library-changed-source55-v4-terminal.json),
SHA-256 `021a36cc9e6311107acf751b1bba7ca6b2bf1e6618c4a32b6b48c17866404012`,
records `failed_scope_sealed` and `client_acceptance: false`. It rechecks UI
logout and exact-token rejection, capability registration, browser/proxy
closure and the exact owned ledger. Both v4 units retain their exact invocations
with exit1, MainPID0 and empty recursive cgroups. The complete old v1/v2/v3
inventories, tools/scopes and six failed units are unchanged; candidate, main,
media and all old rows/sequences/private state are preserved. No additional
cleanup was needed; the seal made no HTTP, business SQL or service writes.
The v4 scope inventory SHA-256 is
`2618849ab3caa0d8756f372e0a3c9022f6d3bc7b2ef325a8e623bcd27172e407`.
V4 is independently sealed and consumed and must not be replayed.

TOOL05/v5 bound actual pins to the complete sealed v4 terminal, ledger and
independent snapshot. Its discovery design uses the complete query for
the single-Movie library, its actual wire response and the unique visible card.
Each change window retains the original same-library Items read or the exact
`/Users/{viewer}/Items/{target}` read, bound to MessageId, route, document and
token. The scope excludes detail navigation, PlaybackInfo, media and Playing
requests. Its normal budget remains two sessions, one device, six audits and
two metadata revisions.

V5's [initial verification](client-library-changed-source55-v5-initial-guards.json)
retains the passing 559 loader, 64 driver and 127 actor cases, plus the controller
suite's original 73 passes/one error from an inconsistent old fixture. Its full
failed report SHA-256 is `ce6de3f403e40d5408e06a1b813c3b0886ba398ca3698b972587cbfcc5d0df9d`.
The separate [05b controller verification](client-library-changed-source55-v5-controller-guards.json),
SHA-256 `100802bbdfa57812bc1890cdd3df9f0a169b5f4b5755be1184edc816b47899fe`,
corrected that fixture and added a contradictory-state rejection case; all 75
passed. The controller remained byte-identical at
`9a699ccf5fce8b73d0d31a32fb030cf57d24a078041c7af32298be21d7ac2c32`, with the old
failed guard and TOOL05 preserved. [Preflight](client-library-changed-source55-v5-preflight.json)
and [real-document checks](client-library-changed-source55-v5-real-documents.json) passed.

The [actual v5 run](client-library-changed-source55-v5.json) and
[browser report](client-library-changed-source55-v5-browser.json) nevertheless
failed in discovery, before any native edit. The
[DOM/wire evidence](client-library-changed-source55-v5-discovery.json) records
three geometrically visible `itemsContainer` elements. The collector required
the total to equal one before enumerating cards, so `cards: 0` is a tool result,
not an empty page. The title-button/card/container chain is visibly present,
and the complete Items200 response still contains the unique expected Movie.
The [screenshot](client-library-changed-source55-v5-discovery.png) is byte-identical
to v4: 38,695 bytes, SHA-256
`07cab956ecc046fa0ad2e634a188bf907b15ff289575ab6160521e933ddead10`.
V5 added one session, one device and two audits with revision delta0; B is closed.
The [independent v5 terminal](client-library-changed-source55-v5-terminal.json),
SHA-256 `348be31960bb19a9905c4cef2b6f19df9af25466e28959f00d1bd5e0a6a2183d`,
records `failed_scope_sealed` and `client_acceptance: false`. It verifies the
exact owned ledger, closure and complete history through v5. Old v1-v4 roots,
tools and eight failed units remain unchanged; the four additional guard and
prerequisite directories match their complete before/after inventories. The
initial failed guard, separate receipt for 75 passing 05b cases and original 559/64/127 pass
records are preserved. Both v5 units retain their exact invocations with exit1,
MainPID0 and empty recursive cgroups. Main, candidate, media and all old
rows/sequences/private state are preserved, with no additional cleanup needed.
The v5 inventory SHA-256 is
...
requirements. Its H6 binding preserves the complete two-session failed-run
...
service mutation. This remains history-input evidence.
The frozen v7 sources passed [865 JavaScript guards](client-library-changed-source55-v7-js-guards.json)
(127 actor, 66 driver and 672 loader) and
[87 Python guards plus two compilation checks](client-library-changed-source55-v7-python-guards.json),
for 952 guards total. All [13 owned-HTML DOM cases](client-library-changed-source55-v7-dom.json)
and the [DOM terminal](client-library-changed-source55-v7-dom-terminal.json) passed,
as did the [actual read-only preflight](client-library-changed-source55-v7-preflight.json)
and [six real history-document checks](client-library-changed-source55-v7-setup-diagnosis.json).

The [actual v7 report](client-library-changed-source55-v7.json),
...
PID1476913/start ticks14431246, and worker invocation
`e87408a7832e4f569983d7789123b9e9`, PID1476974/start ticks14431544. At the latest
observation, both units exited with status 1 and MainPID0. Discovery/armed, validated native
login200 and forward PUT200/readback succeeded, but the first automatic-refresh
window reported `automatic_http_not_observed_within_window`. The final recorded
phase is `restore-armed`: the controller completed native restore200 and confirmed
restoration. It reports two sessions, one device, six audits and two metadata
revisions, with both sessions closed. The
[independent v7 terminal](client-library-changed-source55-v7-terminal.json),
SHA-256 `daec6f02a1a611553a7312a4affb892c5defdbae760205bdd31431827f12118f`,
records `failed_scope_sealed` in phase `restore-armed` and confirms the exact
2-session/1-device/6-audit/revision-delta2 ledger. Original sparse and effective
metadata are restored, both credentials are closed, and old rows/sequences/private
state, main, candidate and media are preserved. All 38 pre-start preservation
roots, 12 old failed units, two old DOM units and the current DOM unit remain
intact. No additional cleanup was needed or performed; the sealer issued no new
HTTP or business SQL. Inventory SHA-256 is
`32e3cdd5dc838114513eb2b117884b5d8522a25b5d734c951e5f83fba576d55c`.
V1-v7 are consumed. Complete LibraryChanged acceptance remains false.
...
binds six original public artifacts. Complete physical WebSocket delivery and
page-frame receipt share MessageId, with only the target in `ItemsUpdated` and
other item arrays empty. All 252 actor requests and 251 proxy requests were
checked by time; both complete channels contain zero requests in the forward
window. All 237 DOM samples retain the old title. Page errors, collection
overflow, observer errors, proxy rejections and proxy failures are zero.
There was no existing request for the catalog matcher to miss. Frame reception
does not establish execution of the original client's internal handler, and
the protocol cause remains open.
...
example and a Name-only UI control. The frozen v1 original-client control's
...
| V4 tool verification | SHA-256 |
| --- | --- |
| [Python04b: 57 guards and two compilation checks](reference-library-changed-ui-python-verification-04b.json) | `3964020055e29180f60295f4b6064c303234f7c46e7220424139e97aedd0f58c` |
| [Driver04: 29 guards](reference-library-changed-ui-js-verification-04.json) | `46c518a7edd2102e95f69449d561d5ae2ab1f8476b32f00789bcddbd9ad8fb31` |
| [DOM04: 13 isolated Chromium cases](reference-library-changed-ui-dom-verification-04.json) | `d2406d2a7a001e709fae9f117289704a8bc77a1e40512ed7af429b7a5c906b6f` |
| [Reused v3 runtime verification: 28 guards](reference-library-changed-ui-js-verification-03.json) | `228f9f4ecfa262f0ffb25cd9a83e4a5b437f515de62943a02ec1ff585bbcb187` |

| Frozen changed v4 source | SHA-256 |
| --- | --- |
| `observe-client-library-changed-reference-ui.py` | `5e021df408e287695c243ab4ec14bd4519e4ca5e3660234605339b030a22105c` |
| `test-observe-client-library-changed-reference-ui.py` | `f892d20345679197db4625472c2261ecd1ab00355c22382264dc292472dcefab` |
| `client-browser-library-changed-reference.mjs` | `965e5e2f0d4a35ef8402b2a79660e0be114344073d282f52886733b06b1f5c5c` |
| `test-client-browser-library-changed-reference.mjs` | `dc5ea5be7c79afa35540caf954638d941c0e84a94e49ffdf747b84cdb72bb029` |

The remote assembly is retained under
`/opt/goby-test/exec-work-m3e/reference-library-changed-ui-tool-04`:
`assembly.json` SHA-256
`5353a2544e55a67042cf9d38b6e6c1c27367ad18827e28572ab681c4fcbf040f`,
and `source-closure.json` SHA-256
`27ab83707db725f200f85f22e84c699fd85035170c975ab9ff65392cb3c6fd2c`.
The private before-state evidence remains under
`/opt/goby-test/exec-work-m3e/reference-library-changed-ui-execution-04`:
`capture-preservation.py` SHA-256
`468417ffc87db29ea6bba3c33e8c5e0f46180c67d75f74f2e044a7be03305a9d`,
`preservation-before.json` SHA-256
`0135f5d39face01900f78402df55f6a0e3d352e63af9ec03f7a6b3d00b0c4aff`,
and `goby-before.json` SHA-256
`e52295a97810b1591a845c493f7504e54cf5f88d985651cc795f63e91e135db6`.
These preparation records do not establish business preflight or actual UI
acceptance. The private state records are not imported into Git.

The [v4 business preflight report](reference-library-changed-ui-preflight-v4.json),
SHA-256 `b9af923afb485c78cb2fb8659ac4d733228c2fa4565eb6a3289e27626fd33e4b`,
and [independent terminal](reference-library-changed-ui-preflight-v4-terminal.json),
SHA-256 `7bfdae06ca09cf3b1eeb1e66718eda01ec28f954a0b0498045fe9a6b2a19ad38`,
record 65 HTTP requests, two authentication POSTs, zero metadata POSTs and
administrator logout204/same-token401. Invocation
`f696c0b6c36a4c8fa0cdeb00b1d29d4b` exited0 with MainPID0 and an empty recursive
cgroup. Under execution04, `preservation-after-preflight.json` SHA-256
`a80934e8599e85afb75f07c7f86754971997a0dcfcd2282fb06b7393f65fd0e0`
confirms all 87 roots unchanged; `goby-after-preflight.json` SHA-256
`642eba2beabed009797025d07025bdec44cc75bfec5a592cd22fbb4b164e8253`
retains the full v7 state at 83 sessions/70 devices/185 audits.

The [independent v4 terminal](reference-library-changed-ui-v4-terminal.json),
captured at `2026-09-12T16:43:45.386028+00:00`, records
`protocol_observation_complete_independently_confirmed`. Controller invocation
`2507f713aa6d47c58f2ac067cb306b19` and worker invocation
`1c804ccfe84b45c9a5d5d1531615ebae` both exited0 with MainPID0 and empty recursive
cgroups. The actual scope issued 78 API HTTP requests: two authentication
POSTs, two metadata POSTs and seven actual-viewer GETs are explicitly counted.
Forward and restoration each returned POST204. Matching-context readback
confirms the complete target restored except Etag, including
`MediaSources[0].Name`; anchor and media remain unchanged. Administrator and
viewer both completed logout204 with independent rejection of the exact token
by401. V4 is complete and consumed; do not replay it.

| Actual v4 evidence | SHA-256 |
| --- | --- |
| [Controller report](reference-library-changed-ui-v4.json) | `100d9d314152daa5cd61985719b81f2cea427c7505d0e2370fb32056ff96550c` |
| [Browser report](reference-library-changed-ui-v4-browser.json) | `09afbb6fdca64fb517b2760ee447d03d7d030264ed243b1dcec7786525a2edef` |
| [Bound input](reference-library-changed-ui-v4-input.json) | `d0ce55bf9254ddb3e82271aa5f8defcd921a2f50d9b1704cbc1907f418632a00` |
| [Independent terminal](reference-library-changed-ui-v4-terminal.json) | `90f6f197e00ab48e9bedf252e5f0534a7e5d8c23a326a33d490e56d446ffa64a` |

Each 120-second observation window contains 225 DOM samples, one physical
LibraryChanged event and one matching browser event. Both complete HTTP
channels contain zero requests throughout each window, and every sampled title
remains `LibraryChanged Observed`. Forward records
`automatic_refresh_not_observed_within_window`. Restoration records
`matches_expected_without_proven_transition`: the unchanged title matches the
restored baseline, so this is not a positive DOM transition.

Execution04 retains `completed-scope-files.json` SHA-256
`7e246418ddf69f40571ee641f15be4884a424786c4965df63b26feee45cc3e32`,
`preservation-after-reference.json` SHA-256
`50914b840c2c8492223009a0c8fa9fb4d4f5d9426ce11eb57a124c8c4db1781d`,
and `goby-after-reference.json` SHA-256
`14aae24782efaea6ef594cb7cb18bf5fdbfb271b2159873c914632036a02fcb3`.
All 87 history/tool roots and the complete Goby v7 state at
83 sessions/70 devices/185 audits remain unchanged.

The [public-message comparison](reference-library-changed-ui-v4-goby-comparison.json),
captured at `2026-09-12T16:45:33.301790+00:00`, has SHA-256
`e8d872896580f965c49b032318b1f5e3309b1ccb96d68760928eaf330f151f36`.
After normalizing only target ID and MessageId, reference v4 and Goby v7 have
identical public envelopes and Data: the envelope contains only `MessageType`,
`MessageId` and `Data`; `ItemsUpdated` contains only the target, the other five
arrays are empty, and `IsEmpty` is false. The 228-versus-257-byte wire difference
is exactly the 29-byte target-ID length difference. V7's three additional summary
fields are absent from the wire; reconstruction matches the original wire hash.
The reference events reached the page 40003ms and 40005ms after POST completion,
compared with 2.0664ms for Goby. This timing difference has no established causal
explanation. The completed negative control closes the missing complete
reference observation, but neither proves a Goby-specific protocol defect nor
substitutes for the positive automatic-HTTP-and-DOM gate. The acceptance target
...
`ee7ea2c3816616f6b954c9a92596c24b9e3f594f`. The
[helper build and 15 pure race tests](main-schema28-helper-verification.json)
passed, and its [owned executable mode](main-schema28-helper-mode.json) is recorded.
The [preliminary read-only main observation](main-schema28-readiness.json)
found PostgreSQL5432/schema27, 102 sessions, 14 devices, 19 audits, five libraries
and roots, four application keys and an existing 32-byte master key. This is not
a complete main baseline. The [service addendum](main-schema28-readiness-service-addendum.json)
corrects `EnvironmentFiles` to two entries. Reuse the existing
`/opt/goby-test/exec-work-m3e/main-deployment-schema25.lock`; the absent historical
M5j lock must not be created. The Python controller passed
[51 remote guards and two compilation checks](main-schema28-python-verification.json) in
`/opt/goby-test/exec-work-m3e/main-schema28-source55-python-verification-01/report.json`,
SHA-256 `50e43ce547acda73c6e1e33b72ed026bd50771efeb66636eecc9d5e6b467cf23`;
raw guard output SHA-256 is
`8a50814aa878607946fac928ea4d0049b41dfc1ede1a95f0173fc19bb990eec6`.
The two Python files and three safe verification records were published as
`ee076930ccc7303e07f39c4a6dd322da051e30e9`; all five matched the frozen raw bytes.
This publication contains tooling and verification evidence only.
The [first read-only main preparation](main-schema28-prepare-attempt1.json) returned `guard_rejected` before creating
a baseline or deployment directory. It made no business SQL, HBA or service
writes; the main and three protected processes are unchanged. Its safe report
is `/opt/goby-test/exec-work-m3e/main-schema28-source55-prepare-verification-01/report.json`,
SHA-256 `ddf8fcf791e01a32e3393ebafb81080c126875d822e076950bd6131a49f32154`.
The [remote catalog diagnosis](main-schema28-prepare-catalog-diagnosis.json),
SHA-256 `c6a93eafa032a2c948e86030ae6a7699f597265f4c8e72cd9316887fe0b62ab6`,
confirmed a representation error: the outer `value` was a string while the
trusted catalog was a list. Exact decoding produced the same 1,109 objects and
canonical SHA-256 `e9d77790b352a739876e621b7a435e0716e7ea62436e7341748da73e1af90850`.
There is no catalog drift. This diagnosis read only catalog data, with no
business-row reads or writes, and did not retry preparation. The two Python
files were corrected as TOOL02 with explicit `::jsonb` and strict type guards.
[54 remote guards and two compilation checks](main-schema28-tool02-python-verification.json)
passed, with report SHA-256
`2fd4edfd4ed0012e0ed3811522dae1b01eddba6c7f71d649d11a6f5d037beb8b`.
Independent review passed. The existing
Go helper's 15-test/build evidence is retained unchanged, including its actual
TOOL01 build arguments. TOOL02 binds that original build provenance and
the copied binary. The TOOL02 correction, verification report, v4 terminal and
related documentation were published in the ten-file commit
`2e83a7c38dc21435495b6f7c40d11f4543838064`.

The [second fresh read-only preparation](main-schema28-prepare-passed.json),
SHA-256 `74581af9566ef4bb0b3f5491b0f03d6cde64f92466790c6f172380f286fc7452`,
passed with the original CLI exiting0 and returning `prepared` for run
`20260912_121742_14953df92aa5`. Its prepare-input SHA-256 is
`343a283642b754648e148aef1020faec7c1003948c54f279dae53b9809d29a44`.
Two complete captures passed a strict comparison. Main PID762090, the three
protected processes, TOOL02, old TOOL01 and the first failed preparation remain
preserved, with zero business SQL, HBA or service writes. The private baseline is
`/opt/goby-test/exec-work-m3e/main-schema28-source55-prepared-20260912_121742_14953df92aa5/baseline.json`,
SHA-256 `4390cd27b77a28b732438a0a63d7f26b3e610e96d976ccb2f106f26918fbe2aa`.
...
Main TOOL03 now distinguishes the controller's exact five-key authority from
the browser/input shape with its additional `before_snapshot`, binding that
snapshot to the original bytes of the scope's `before-full.json`. It passed
[55 Python guards and two compilation checks](main-schema28-tool03-python-verification.json)
and independent review without a blocker. The three files were published as
`45ebb226a1b4315e5f51855ea6e6d797df76469b`. Old TOOL02 and the expired preparation
remain preserved; this verification did not execute a fresh preparation or main
upgrade. The main service remains source32/schema27.
...
The TOOL05 producer was published as `609fa9180c30c4f9592303450563a5646be96857`
after 114 remote guards and two compile checks. Its
[actual preparation02](nextup-global-reference-preparation-02.md) passed plan and
read-only admission, then stopped after 33 HTTP attempts on the Devices zero-count
sentinel. Independent cleanup confirms the administrator token closed, six
retained media copies, one retained authentication device, no library/user/playback
operations, 109 old roots unchanged and the full Goby state at 83/70/185 preserved.
Configuration was never observed; the partial capture is not a new full baseline.

The corrected TOOL06 source passed [140 remote guards](nextup-global-preparation-verification-03.json)
and two compile checks. Its Devices-only decoder preserves the complete old
registry and requires exactly the newly acknowledged login devices. Catalog
pagination remains strict, and durable pending-ownership semantics are unchanged.
...
The accepted schema28 upgrade and subsequent client continuation chain are recorded above;
new candidate work must account for the complete chain rather than reuse this historical baseline.
...
independently sealed. V7's corrected tools passed their prerequisites and the
...
within the window. Reference UI v3 passed its tool checks, actual preflight,
viewer login, discovery and armed stages, then sent one forward metadata POST204.
The unmodeled derived media-source name and mixed authentication baseline stopped
complete observation. Independent recovery restored the full target except Etag
and preserved anchor/media; the original failed report remains unchanged.
Both original sessions and the recovery administrator have verified closure.
The v3 terminal is `failed_scope_sealed_after_independent_restoration`,
with 74 preserved roots and unchanged post-v7 Goby state. V4 subsequently passed
remote tool verification and independent preflight, then completed both
observation windows, restoration and independent cleanup. Its terminal records
`protocol_observation_complete_independently_confirmed`, with 87 roots and the
complete Goby v7 state preserved. Both windows have zero automatic HTTP and no
title transition. This complete negative control does not satisfy the positive
client gate or establish a Goby-specific defect. V2's historical dispatch
uncertainty remains intact. Complete LibraryChanged acceptance remains false.
...
corrected preparation producer passed 33, 91 and 140 remote guards respectively,
plus their compilation checks. Actual preparation02 stopped after 33 HTTP attempts
and was independently sealed after known cleanup. Six media copies and one closed
administrator device remain; no new library, user or calibration was created.
The later observer02 completed all 34 requests and independent acceptance. Its
complete baseline and closed-authentication row now support fresh preparation03.
Observer149 and operator57 guards and compile checks are published. Use the
dedicated future matrix parent before freezing preparation inputs. SSH is
available; actual matrix/client acceptance remains open.
