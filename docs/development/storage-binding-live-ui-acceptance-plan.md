...
3. Retain the corrected preparation producer's 140 remote guard passes and the
   ...
   playback. Complete the new 34-request baseline observer and independent
   closure through `ssh test-env`, then use a fresh admitted preparation scope.
   Reference capture and client playback still require the reviewed outer
   operator. TOOL02's live CLI remains disabled. Do not substitute local tests.
