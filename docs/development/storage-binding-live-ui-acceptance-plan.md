...
Current checkpoint: `2026-09-12T18:04:23+00:00`; runtime, transport, preparation tools and all four owned-browser controls are published and remotely verified through `609fa9180c30c4f9592303450563a5646be96857`. Real NextUp preparation has not executed, new SSH connections are blocked at agent signing, and the positive client gate is unmet. The storage-binding UI gate below remains accepted.
...
3. Retain the published preparation producer's 114 remote guard passes and the
   four passed owned-HTML callback/HTTP/DOM controls. Complete the private
   NextUp inputs, then run the frozen plan and read-only authority admission
   through `ssh test-env` after SSH agent signing recovers. No new six-copy
   fixture, library, user or calibration exists. Actual preparation requires a
   fresh bounded scope, complete preservation and independent attestation;
