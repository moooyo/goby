...
| M0 core reference capture | Baseline complete; broader coverage pending | Official Emby 4.9.5.0 evidence contains 2305 records. The [fresh configuration mutation study](../research/configuration-mutation-reference.md) adds 254 after the [read study](../research/configuration-reference.md) reached 2051. Earlier device, key, task, catalog, and media evidence is preserved. Broader configuration writes, changed-value key writes and restart persistence, task timer/key-auth behavior, weekly/system-event execution, DST/maximum-runtime enforcement, global NextUp selection, and hidden header-device Info/deletion remain unresolved. Reference records are separate from product acceptance |
...
| M5 administrator completion | User, metadata, login-session, application-key, device, scheduled-task and native settings increments complete; milestone incomplete | [M5a](verification-m4e-video-and-users.md), [M5b](verification-m5b-metadata.md), [M5c](verification-m5c-sessions.md), [M5d](verification-m5d-application-keys.md), [M5e](verification-m5e-devices.md), and [M5f](verification-m5f-tasks.md) retain prior acceptance evidence. [M5g native settings](verification-m5g-settings.md) passes 1222 top-level race tests across fourteen packages, browser/restarts, protected deployment, and the main-service workflow. ConfigurationService compatibility, additional task executors, full policies, providers, broader settings, audit/log browsing, product backup/restore and broader wire/client parity remain |
...
| M5g native settings | Native increment complete; ConfigurationService adapter pending | [Verification](verification-m5g-settings.md): 1222 full-race tests across fourteen packages, browser with two exact 28-table restarts, protected schema-20 deployment, and first-pass settings/restore workflow on the main service. Three [native routes](../api/settings.md) manage five nullable overrides with CAS and atomic publication. The [read](../research/configuration-reference.md) and [fresh mutation](../research/configuration-mutation-reference.md) studies retain bounded reference evidence, not complete compatibility |
...
adds 254 records: 17 setup records and 237 capture records, bringing the current
corpus to 2305. Its 236 complete HTTP exchanges and audit record 145,347
...
Goby's ConfigurationService adapter remains unimplemented.
...
The service uses an operator-provisioned private master under `GOBY_API_KEY_MASTER_KEY_FILE`, with UID 995, mode `0600`, 32 bytes, and a mode-0700 parent. [Application-key operations](application-keys.md) require preserving the database and matching master together; secret recovery is not a shipped backup/restore or rotation workflow. Earlier video restart, refresh, session, key, device and task increments remain in place. ConfigurationService compatibility, additional task executors, broader settings, providers, audit/log browsing, product backups, actual hardware execution, and full client acceptance remain open. This is a test deployment, not a public production release; M5g native settings is a completed increment while M4, M5, M6, and the full goal remain unfinished.
