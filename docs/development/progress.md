...
| M0 core reference capture | Baseline complete; broader coverage pending | Official Emby 4.9.5.0 isolated on test-env, 1128 audited records including HTTP/WebSocket/media observations. [Key playback](../research/api-key-playback-reference.md), [client contexts](../research/api-key-context-reference.md), and [target scope](../research/api-key-scope-reference.md) add 122 records with 119 complete HTTP exchanges. They distinguish independent credentials, multiple clients, explicit-user library ACLs and conversion permissions; implementation verification is in progress. Global NextUp selection remains unresolved; [HTTP](../research/reference-server.md), [WebSocket](../research/websocket-reference.md), [HLS](../research/hls-reference.md), [audio](../research/audio-reference.md), [audio profiles](../research/audio-profile-reference.md), [video profiles](../research/video-progressive-reference.md), [metadata](../research/metadata-reference.md) |
...
The current service is active as the unprivileged `goby` user at `http://127.0.0.1:18096` on the test host, with schema 15 and probe version 6. M4g passed 969 top-level tests across all twelve tested packages with zero skips, real-media browser/restart acceptance and five deployed library refreshes. The upgrade preserved every old business field and defaulted historical job modes to false. Live verification re-probed all eleven media files while retaining metadata, user state, old tasks and authentication; its single new administrator login was revoked. M4f video restart and M5c session management remain in place. Credentials remain in root-only environment files on that host. It is a test deployment, not a public production release.
