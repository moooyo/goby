...
| M0 core reference capture | Baseline complete; broader coverage pending | Official Emby 4.9.5.0 isolated on test-env, 436 audited HTTP/WebSocket/media records, including positive HLS transcode delivery and preserved remux failures; global NextUp selection remains unresolved; [HTTP report](../research/reference-server.md), [WebSocket report](../research/websocket-reference.md), [HLS report](../research/hls-reference.md) |
...
| M4 conversion and hardware pipeline | Engine and HLS VOD increments complete; milestone incomplete | [M4a verification](verification-m4a-engine.md): real conversion engine and unprivileged execution. [M4b verification](verification-m4b-hls.md): authenticated full VOD graph, source-global seek production, current conversion policies, startup configuration and schema 11; complete Linux race suite plus deployed continuous playback, seek, authorization and cleanup passed. Progressive/universal audio, additional tracks/formats, hard resource controls, actual GPU decode/encode and full client acceptance remain |
