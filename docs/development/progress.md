...
| M5f scheduled tasks | Read-only reference and internal transaction foundations complete; product incomplete | [Reference study](../research/scheduled-tasks-reference.md): 128 complete HTTP exchanges and one audit, 22 task definitions, read permissions and trigger declarations. [Foundation verification](verification-m5f-task-foundations.md): 21 targeted race tests pass for owned transactions and live administrator checks. The [task plan](../research/scheduled-tasks-plan.md) still requires repositories, execution, schedules, HTTP and dashboard integration. No scheduler, browser, restart, or deployment acceptance is claimed; task execution and trigger mutations remain unsampled |
...
not product task execution. The separate [transaction foundations](verification-m5f-task-foundations.md)
now pass 21 targeted Go race tests on the isolated Linux database. They add
restricted owner transactions and live administrator authorization without
registering task routes or schedules. The deployed M5e `d5d696f`, schema 18/probe 6, and its
