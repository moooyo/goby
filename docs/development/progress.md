...
| M0 core reference capture | Baseline complete; broader coverage pending | Official Emby 4.9.5.0 isolated on test-env, 153 audited HTTP fixtures including NFO/artwork, entities, original playback, and recursive watched state; selected responses corrected from these captures; [report](../research/reference-server.md) |
...
| M3 initial client playback | Original playback/state increment complete; milestone incomplete | Full Linux race/integration tests and deployed ten-minute H.264/AAC decoding passed; [M3a verification](verification-m3a-original-playback.md). External subtitles, NextUp, client-session capabilities/events and real-client acceptance remain |
