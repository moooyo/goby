...
| M2 media catalog | In progress | Media probing has started; scanner, metadata, images, queries, permissions and administrative library workflow remain |
...
The foundation service is active as the unprivileged `goby` user at `http://127.0.0.1:18096` on the test host. Credentials remain in root-only environment files on that host. It is a test deployment, not a public production release.
