...
Historical tool03 state: **it completed the scan (`Completed`, 6/6/0),
then failed an incorrect count assertion after 15 valid protocol responses.**
Job `d7aa0acaee023dd4c82ea7a303c354ca` used the existing library/root. Inherited
`84af.capture` required `SpecialFeatureCount=3`; the correct sampled response
omits that field and supplies `LocalTrailerCount=1`, as the reference 20-case
capture records. This tool assertion does not justify changing product behavior.
The [continuation failure](m3e-positive-continuation-count-check-failed.json)
remains retained. Both new admin/viewer sessions completed logout204/exact401.
Full/range media requests had not started.

The [independent response proof](m3e-positive-protocol-response-proof.json)
passed six direct reads, eight lists and the parent response: all 15 complete
HTTP200 with correct membership, paths, owner and media sources. The [snapshot review](m3e-positive-protocol-snapshot-review.json)
passed original 13-item/all-old-row preservation, new 9 items/four Extra resources/
three markers, three revoked actor sessions, exactly nine new audits and exact
sequences. At that failure the candidate had 35 tables, 22 items, 4 libraries, 5 UserData
rows, 24 plays, zero encoding jobs, 61 global sessions and 135 audits. Its retained failed state is
...
PID748513/start ticks6996875, schema27, source32 binary and runtime are unchanged.

The later independent protocol-finalization/inspection chain reused those 15
responses, completed full200/Range206 delivery for all four resources and
login/logout/exact-token proof in 11 HTTP exchanges, and added one session/device
and two audits while preserving old snapshot rows. No rescan, recreation or
old-tree success marker occurred. Positive UI and its repaired comparison also
passed. Both execution failures, both preflight diagnostics and the first
comparison failure remain preserved. Main consumer03 is historical tooling;
consumer04 passed 27 guards/build, and the primary later completed its separately recorded schema27 deployment.

Historical setup01 state: **it failed after its create201; the library
existed, and that attempt had not run a scan.** The [12 pure guards](m3e-positive-fixture-tool01-guards.json),
...
The subsequently executed continuation bound the failure, create201
...
scan and viewer actors. The independent review now proves the three sessions,
nine audit entries and exact sequences from the original baseline. The scan
completed, but the count assertion stopped full/range checks in that attempt.
Separate finalization and positive UI later passed. See [extras verification](verification-m3e-extras.md)
...
setup01 and tool03. At the setup01 failure the candidate had 14 items (old 13 plus one CollectionFolder),
...
were empty at that checkpoint. The extension's 13-item/three-library result is historical.
...
Full/range finalization, independent inspection and the positive original-client
flow now passed under their bound scopes. Primary backup/rehearsal/migration
and native smoke subsequently passed in one attempt. The later library-restriction/
restore API matrix and inspection also passed. Its original-client UI permission
counterpart and other compatibility cases remain open.
The empty-array Movie pass and added root permission
do not by themselves establish indexed extra delivery; the later positive
protocol/media proof supplies that sampled evidence. Original-client UI permission
changes and complete compatibility remain open after the API matrix pass.
Main consumer04's tooling verification
is separate from the successful primary deployment above. Completed media/reference phases must
not be replayed.

Historical main deployment: [source28/schema26](m3e-main-schema26-completed.json),
formerly active/running as PID688833/start ticks5620918, binary SHA-256
...
scope. The later [library-restriction API matrix](m3e-library-restriction-api.json)
and inspection passed; original-client UI permission changes remain open.
Raw reports and all historical successes/failures remain preserved.
...
the accepted source28/schema26 checkpoint and then the current source32/schema27
primary checkpoint above. No complete M3 compatibility
...
| Sessions and isolation | Real client reports and session changes; a second owned user cannot observe or change the first user's protected state | Original-Movie and positive-Movie A/B flows passed: complete SpecialFeatures200 (empty and three-resource scopes), zero page errors, own200/foreign403, unchanged state projections and logout204/exact401. Positive cards matched unique titles without DOM IDs; one console warning/error per user remains. The repaired scoped database comparison passed. The restriction/restore API matrix and independent persisted-state inspection passed; original-client UI permission changes and broader isolation remain open; old scopes are historical |
...
| Regression and publication | Relevant remote regression, complete final-source checks, reviewed deployment where needed, documentation and commit/push | Source32 full 1,873-test race suite/build, candidate schema27 upgrade, exact root extension, protocol/media finalization, independent inspection and scoped positive A/B UI/comparison passed; product b9bb7b1 is published. Main consumer04 passed two syntax checks/27 guards/build, followed by the independent successful primary schema27 backup/rehearsal/migration/native-smoke deployment. The restriction API/inspection passed; original-client UI permission changes and complete milestones remain open |
