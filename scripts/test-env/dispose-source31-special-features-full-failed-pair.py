#!/usr/bin/env python3
"""Normalize and dispose only the reviewed source31 interrupted schema27 pair.
...
def normalize_reviewed_pair(m, runner, pair, before, checks, publish):
    ...
    publish(OUTPUT / ("disposal-normalize-" + name + "-intent.json"), {"run_id": RUN, "pair": {key: pair[key] for key in ("name", "role_oid", "database_oid")},
    ...
    for schema, statements in checks.items():
        ...
        body += "\n".join("IF (" + query + ") IS DISTINCT FROM E'" + raw.replace("\\", "\\\\").replace("'", "''") +
    ...
    publish(OUTPUT / ("disposal-normalize-" + name + "-completed.json"), {"run_id": RUN, "name": name, "objects": [], "status": "normalized"})
