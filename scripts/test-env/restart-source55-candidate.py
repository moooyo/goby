#!/usr/bin/env python3
"""Start the failed source55 candidate once after both client phases close.

Run remotely as root with Python -I -B and --input-sha256. The precreated
candidate-source55-restart-02/input.json must bind this source, the completed
...
ROOT = W / "candidate-source55-restart-02"
...
CLIENT_EXECUTION = W / "reference-nextup-client-discovery-execution-02"
CLIENT_UNIT = "goby-nextup-client-discovery-02.service"
PREVIOUS_CLIENT_UNIT = "goby-nextup-client-discovery-01.service"
PREVIOUS_CLIENT_CLOSURE = W / "reference-nextup-client-discovery-execution-01/independent-client-terminal-04.json"
PINS = {
    ...
    PREVIOUS_CLIENT_CLOSURE: "f123a6421aa3ff86eda190dc81bd5338b7fcdeb8ac31e09b5555f8c74cd2f001",
    CLIENT_EXECUTION / "preservation-before.json": "19d10cebf550702f02c48ca24baae354928cab847e774c58451dba364cd58723",
    CLIENT_EXECUTION / "preservation-after.json": "2a0c21b02de7e4a8d8def62fe31e1b66deb9ebff7c2ee6fb02516d03618e09ec",
}
OLD_PID = 1458051
...
def show(unit, fields=SERVICE_FIELDS, *, timeout=10):
    require(unit in (UNIT, CLIENT_UNIT, PREVIOUS_CLIENT_UNIT, "goby-foundation-test.service", "goby-emby-client-m3e.service"), "unit-query-scope")
...
def main():
    ...
    previous_closure = json.loads(read(PREVIOUS_CLIENT_CLOSURE, PINS[PREVIOUS_CLIENT_CLOSURE]))
    require(closure.get("unit", {}).get("formerPid") == 1811269 and
            closure["unit"].get("invocationId") == "ab5ab603ae8d42ed854cf7ce675c1dc9", "actual-client02-invocation")
    client_closures = {PREVIOUS_CLIENT_UNIT: previous_closure, CLIENT_UNIT: closure}
    for unit, value in client_closures.items():
        require(value.get("kind") == "nextup-client-discovery-independent-terminal" and value.get("version") == 1 and
                value.get("status") in ("client_closed_global_request_unobserved", "client_discovery_evidence_reconstructed") and
                value.get("failure") is None and value.get("clientDiscoveryEvidenceVerified") is True and
                value.get("uiLoginAndLogoutDualChannelBound") is True and value.get("exactUIToken401MetadataVerified") is True and
                value.get("resumeOrRetryAllowed") is False and value.get("unit", {}).get("name") == unit and
                value["unit"].get("formerPidAbsent") is True and value["unit"].get("cgroupAbsent") is True and
                not Path("/proc", str(value["unit"]["formerPid"])).exists(), "client-independent-cleanup-minimum")
    ...
    for phase in ("before", "after"):
        ...
        require(descriptor["sha256"] == PINS[Path(descriptor["path"])], "fixed-client02-preservation-sha256")
        ...
        require(value["kind"] == "nextup-client-preservation" and value["schemaVersion"] == 1 and value["phase"] == phase and
                value["rootCount"] == len(value["roots"]) == 198 and value["historical192RootsEqual"] is True and
    ...
    terminal = {"schemaVersion": 1, "kind": "candidate-source55-restart-terminal", "status": "failed_before_start",
                ...
                "previousClientClosure": {"path": str(PREVIOUS_CLIENT_CLOSURE), "sha256": PINS[PREVIOUS_CLIENT_CLOSURE]},
    ...
    try:
        ...
        stage = "preconditions"
        closed_client_units = {}
        for unit, closed in client_closures.items():
            client = show(unit)
            require(all(client[key] == value for key, value in {"MainPID": "0", "LoadState": "loaded", "ActiveState": "active",
                    ...
                    "InvocationID": closed["unit"]["invocationId"]}.items()) and empty_cgroup(unit) and
                    not Path("/proc", str(closed["unit"]["formerPid"])).exists(), "client-discovery-unit-closed")
            closed_client_units[unit] = client
        ...
        require(protected_runtime() == protected_before and show(UNIT, CANDIDATE_FIELDS) == old_service and
                all(show(unit) == client and empty_cgroup(unit) and
                    not Path("/proc", str(client_closures[unit]["unit"]["formerPid"])).exists()
                    for unit, client in closed_client_units.items()) and
        ...
        records["intent"] = publish("start-intent.json", {"kind": "candidate-source55-single-start-intent", "schemaVersion": 1,
            ...
            "previousClientClosure": {"path": str(PREVIOUS_CLIENT_CLOSURE), "sha256": PINS[PREVIOUS_CLIENT_CLOSURE]},
            "closedClientUnits": closed_client_units,
