#!/usr/bin/env python3
"""Reconstruct retained zero-playback NextUp client discovery evidence.
...
UNIT_FILE_SHA = '699d3b655bbbc3b6e38a75ac32eb4514f0f5eb76c5b895303a03dc418dddc990'
OUTPUT = E / 'independent-client-terminal.json'
FILES = ('client-browser-nextup-discovery.mjs', 'client-browser-nextup-discovery-runtime.mjs', 'client-browser-session-proof.mjs')
SOURCE_PINS = dict(zip(FILES, (
    'b25887f0aec5b0a7ec16abc989f8ee93042d221893a417f123589bc5b09fcead',
...
class Verifier:
    ...
    def recorder(self, phase, offset, matrix, credentials, user, server):
        ...
        device = 'goby-nextup-client-discovery-01-' + phase
    ...
    def browser(self, input_value, matrix, credentials, user, server, report):
        ...
        for row in global_rows:
            ...
            need(len(claimed) == 1 and claimed[0]['actual_client_response_bound'] is True and claimed[0]['frame_id'] == frame['id'] and
                 ...
                 same(claimed[0]['exact_public_query'], row['exact_public_query']), 'claimed_global_response_not_reconstructed')
        self.summary.update(uiTokenSha256=token_hash, uiLoginAndLogoutDualChannelBound=True, exactUIToken401MetadataVerified=True,
                            ui401RawWireReconstructed=False, pageDecodedBodyDigestIndependentlyCaptured=True,
    ...
    def visible(self, report):
        ...
        labels = ('home-after-login', 'before-la', 'after-la', 'home-between-libraries', 'before-lb', 'after-lb')
    ...
    def complete_inventory_and_order(self, report, network, input_value):
        ...
        expected.update('visible-' + label + suffix for label in ('home-after-login', 'before-la', 'after-la',
                        'home-between-libraries', 'before-lb', 'after-lb') for suffix in ('.json', '.png'))
        previous = 0
    ...
    def unit(self, admission, input_value):
        ...
        expected_unit = {'path': str(UNIT_FILE), 'sha256': UNIT_FILE_SHA}
        need(intent['kind'] == 'nextup-client-discovery-launch-intent' and intent['unitName'] == UNIT and
             ...
             intent['maximumSeconds'] == 1200 and intent['permittedPlaybackAttempts'] == 0, 'actual_launch_intent_binding')
        ...
        unit_bytes = self.read(UNIT_FILE, UNIT_FILE_SHA)
    ...
    def run(self):
        ...
        need(input_value['kind'] == 'nextup-reference-client-discovery-input' and input_value['version'] == 1 and
             input_value['runId'] == 'nextup-reference-client-discovery-01' and input_value['root'] == str(ROOT), 'input_scope')
...
def main():
    ...
    need(fingerprint(args.input_sha256) and args.former_pid > 1 and re.fullmatch(r'[0-9a-f]{32}', args.invocation_id), 'required_runtime_binding_invalid')
