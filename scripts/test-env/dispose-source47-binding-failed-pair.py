#!/usr/bin/env python3
"""Dispose only the reviewed, empty source47 binding targeted-test pair.
...
RUN = "20260912_040039_8ec465c99d0b"
WORK = Path("/opt/goby-test/exec-work-m3e")
SOURCE = WORK / "source-attempt-47"
OUTPUT = WORK / ("client-backup-run-" + RUN)
EXECUTION = WORK / "storage-binding-schema28-execution-01"
...
MANIFEST_SHA = "a0bb8413680da27cee7f2159bf34eda1405ddff4ae57f2564747717f03bf8831"
RECEIPT_SHA = "57ade271512651ff8a9179f1453385f684c877e5a64febcf9c58f28e4c79b423"
REPORT_SHA = "9bcf5a32a626b1aea89216b6f809e859ac9e758032dc0db5bc63bf433ca2efe1"
LOG_SHA = "88ebe7f72a95f10de52fe422ebb99e6475825080845ce416c4f794e7126d9870"
LOG_BYTES = 843636
TERMINAL_SHA = "8a76222c25bf9c6dfecbd34d070df25b30edc44cb790fc2239ebca291921bfb9"
...
PAIRS = [{"name": "goby_backup_m3e_source", "role_oid": 15921575, "database_oid": 15921576},
         {"name": "goby_backup_m3e_target", "role_oid": 15921577, "database_oid": 15921578}]
OUTPUT_IDENTITY = {"device": 2049, "inode": 2428929}
TAG = "goby-client-backup-pair-m3e-v1:" + RUN
UNIT = "goby-client-backup-20260912-040039-8ec465c99d0b.service"
OUTER_UNIT = "goby-storage-binding-schema28-controller-v1.service"
INNER_PROCESS = {"pid": 1237620, "start_ticks": 10850628,
                 "boot_id": "6bdfc486-7bc8-412f-82b5-70095a09dde7"}
RUN_EXPRESSION = ("^(Test(RootBinding|StorageRootBindingMigration|PostgreSQL|.*Migration|CatalogAudit|HTTPRootBinding|"
                  "HTTPLibraryRefreshDurablyQueuesEveryLibraryBeyondScannerCapacity|"
                  "ManagerRetriesCapacityAndIndependentScansBeyondTerminalPage))")
PACKAGES = ("./internal/database", "./internal/backuppg", "./internal/activity", "./internal/library",
            "./internal/server", "./internal/tasks")
UNIT_PINS = {
    UNIT: {"InvocationID": "2c24383898c14ab3ae497058469a7be8", "ExecMainPID": "1237620", "Description": TAG},
    OUTER_UNIT: {"InvocationID": "6693db24aa2d4c778d8872713701a4ec", "ExecMainPID": "1237445",
        "Description": "[systemd-run] /usr/bin/python3 -I -B " + str(SOURCE) +
        "/scripts/test-env/run-client-backup-tests.py --source " + str(SOURCE) +
        " --manifest-sha256 " + MANIFEST_SHA + " --schema 28 --mode targeted --run \"" + RUN_EXPRESSION +
        "\" --package ./internal/database --package ./internal/backuppg --package ./internal/activity" +
        " --package ./internal/library --package ./internal/server --package ./internal/tasks"},
}
GUARD_SHA = {
...
TREE_SHA = {
    str(SOURCE): "716142a31dcbc3d1ae3520432ac2e6fe06c1c3b22a037e5605f14b0a8f602b4f",
    str(OUTPUT): "7c950e28af399c1962de4b83e08e60fc94eecc5f5d14fca2f9e5794a221e909d",
    str(EXECUTION): "10402f5a278b63afd0d5839c4a3fff046c7fe6dd99da74386a9875f8f8e9e915",
}
...
def load_runner():
    ...
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or hashlib.sha256(RUNNER.read_bytes()).hexdigest() != RUNNER_SHA:
        raise RuntimeError("The pinned runner differs from the reviewed source47 executable.")
    spec = importlib.util.spec_from_file_location("source47_disposal_runner", RUNNER)
...
def main():
    ...
    m.require(files[m.catalog_name(28)] == CATALOG_SHA and
              ...
              "The source47 executable or exact schema28 catalog changed.")
    ...
    try:
        ...
        raise RuntimeError("Source47 disposal stopped; private evidence is retained and retry is forbidden.") from None
    finally:
