...
Status: prepared source; not executed. A context file, saved preference, returned
...
The fixture serves its existing `system/dashboard-ui` read-only through the
normal `WebDirectory` configuration. The package is not copied into this
repository, browser results, build assets, or a redistribution archive. Record
its asset inventory and hash alongside the frozen product source. The observed
client version is 4.9.5.0; a different asset tree requires a new contract review.

## Admission and execution

No local tests, builds, format validation, browser runs, or media probes are
authorized by this guide. All commands below belong to the coordinator's
single admitted remote verification slot after all phase code is integrated.
