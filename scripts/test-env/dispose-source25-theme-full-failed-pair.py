#!/usr/bin/env python3
"""Dispose only the independently reviewed, empty source25 full Theme test pair.
...
RUN = "20260911_102522_aa52920473bf"
WORK = Path("/opt/goby-test/exec-work-m3e")
SOURCE = WORK / "source-attempt-25"
...
MANIFEST_SHA = "8ec6ffbe1675bc7db036d4741e0952641dbc444bc23ff046b3f4aeac5656baed"
RECEIPT_SHA = "fde6283a1a42da1dc33bc620b06ada6534ce0f0679bb8eca498b97e4641f72ce"
REPORT_SHA = "7799534ebbfd527072bb671fc3911c53f150c4b3c5968a3acfed33c2216b883f"
LOG_SHA = "0359d359e44ae664b502996744cf9f091d546c5a9b4cd33e9c868bf1efbd314e"
...
PAIRS = [{"name": "goby_backup_m3e_source", "role_oid": 5236525, "database_oid": 5236526},
         {"name": "goby_backup_m3e_target", "role_oid": 5236527, "database_oid": 5236528}]
OUTPUT_IDENTITY = {"device": 2049, "inode": 3160539}
OBSERVED_PUBLIC = {
    ...
    "goby_backup_m3e_target": {"oid": 5791149, "owner": "pg_database_owner",
        "acl": ["pg_database_owner=UC/pg_database_owner", "=U/pg_database_owner"],
        "comment": "standard public schema"},
}
...
def load_runner():
    ...
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or hashlib.sha256(RUNNER.read_bytes()).hexdigest() != RUNNER_SHA:
        raise RuntimeError("The pinned runner differs from the reviewed source25 input.")
    spec = importlib.util.spec_from_file_location("source25_full_disposal_runner", RUNNER)
...
def main():
    ...
    try:
        ...
        def inspect_pair(pair):
            ...
            m.require(observed_public == OBSERVED_PUBLIC[name] and runner.cast_inventory(name) == pair["casts"],
            ...
            return {"identity": rows, "original_public": pair["public"], "observed_public": observed_public,
                    "catalog_objects": [], "table_row_counts": {},
