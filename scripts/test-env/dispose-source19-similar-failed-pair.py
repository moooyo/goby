#!/usr/bin/env python3
"""Dispose only the independently reviewed, empty source19 Similar test pair.
...
RUN = "20260911_075321_2bae7b2afb21"
WORK = Path("/opt/goby-test/exec-work-m3e")
SOURCE = WORK / "source-attempt-19"
...
RUNNER_SHA = "eb3e12e6a6a3dec71db95fdb29c642232728e96c6d6dcd0c903adf6b507a793d"
MANIFEST_SHA = "cdf8775d7ffd0973046d83fef48d8ab3a813105164f807755ae9783a18545708"
RECEIPT_SHA = "d7ad543a3438298ba63993deed7a23b0cb68c26adb83b09032e50ef324550e0f"
REPORT_SHA = "3fd116765f17978e70a1bb53202281ae0d1df5d608b555dc28962ca476793199"
LOG_SHA = "a1cae0248fa6a2e8004b4012d1ad0ee4a35a068795ca97b35407822088ee46b6"
...
PAIRS = [{"name": "goby_backup_m3e_source", "role_oid": 4252376, "database_oid": 4252377},
         {"name": "goby_backup_m3e_target", "role_oid": 4252378, "database_oid": 4252379}]
OUTPUT_IDENTITY = {"device": 2049, "inode": 2426512}
...
def load_runner():
    ...
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1 or hashlib.sha256(RUNNER.read_bytes()).hexdigest() != RUNNER_SHA:
        raise RuntimeError("The pinned runner differs from the reviewed source19 input.")
    spec = importlib.util.spec_from_file_location("source19_disposal_runner", RUNNER)
...
def main():
    ...
    identity, files = m.verify_source(SOURCE, MANIFEST_SHA, 25)
    ...
    try:
        ...
        m.require(receipt["marker"] == m.MARKER and receipt["run_id"] == RUN and receipt["tag"] == tag and
                  ...
                  receipt["source_manifest_sha256"] == MANIFEST_SHA and receipt["schema"] == 25 and
        ...
        args = SimpleNamespace(source=SOURCE, manifest_sha256=MANIFEST_SHA, schema=25, mode="targeted", run="", package=[])
