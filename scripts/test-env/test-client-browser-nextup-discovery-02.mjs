...
import { validateInput, validateFreshOutput, ownedStateRoutes, compareOwnedState, summarizeNextUp, suggestionsSelection, assertSafeExport, WORK, ROOT, TOOL, FILES, EPISODES, SUMMARIES }
  from './client-browser-nextup-discovery-02.mjs';

const checksum = 'a'.repeat(64);
const description = filename => ({ path: WORK + '/' + filename, sha256: checksum });
function input() {
  return { kind: 'nextup-reference-client-discovery-input', version: 1, runId: 'nextup-reference-client-discovery-02', root: ROOT,
    ...
    continuation: { priorReport: description('reference-nextup-client-discovery-01/private/report.json'),
      priorIndependentTerminal: description('prior-independent.json'), priorElapsedMilliseconds: 10586, totalMaximumSeconds: 1200 },
    budgets: { maximumSeconds: 1189, normalSeconds: 829, maximumPlaybackAttempts: 2, permittedPlaybackAttempts: 0,
      recorderRequests: 34, recorderBodyBytes: 2097152, browserNavigationActions: 5 } };
}
...
test('reject scope, source, credential, and budget widening', () => {
  for (const change of [value => value.budgets.permittedPlaybackAttempts = 1, value => value.root += '-retry',
    ...
    value => value.continuation.priorElapsedMilliseconds = 0, value => value.budgets.maximumSeconds = 1200,
    value => value.budgets.normalSeconds = 840, value => value.budgets.browserNavigationActions = 6,
    value => value.runId = 'nextup-reference-client-discovery-01']) {
    const value = input(); change(value); assert.throws(() => validateInput(value));
  }
});
test('record Suggestions selection only from explicit markers or transferred selection classes', () => {
  const before = { shows: { class_name: 'tab emby-tab-button-active' }, suggestions: { class_name: 'tab' } };
  const after = { shows: { class_name: 'tab' }, suggestions: { class_name: 'tab emby-tab-button-active' } };
  assert.equal(suggestionsSelection(before, after).selected, true);
  assert.deepEqual(suggestionsSelection(before, after).transferred_selection_classes, ['emby-tab-button-active']);
  assert.equal(suggestionsSelection(before, before).selected, false);
  assert.equal(suggestionsSelection(before, { shows: {}, suggestions: { aria_selected: 'true' } }).selected, true);
  assert.equal(suggestionsSelection(before, { shows: { aria_selected: 'true' }, suggestions: { aria_selected: 'true' } }).selected, false);
  assert.equal(suggestionsSelection(before, { shows: { class_name: 'emby-tab-button-active' }, suggestions: { aria_selected: 'true' } }).selected, false);
  assert.equal(suggestionsSelection({ shows: {}, suggestions: { class_name: 'emby-tab-button-active' } },
    { shows: {}, suggestions: { aria_selected: 'true' } }).selected, false);
  assert.equal(suggestionsSelection({ shows: {}, suggestions: { aria_selected: 'true' } },
    { shows: {}, suggestions: { aria_selected: 'true' } }).selected, false);
  assert.equal(suggestionsSelection({ shows: { class_name: 'focus' }, suggestions: {} },
    { shows: {}, suggestions: { class_name: 'focus' } }).selected, false);
});
...
test('retain original response order and query without a SeriesId', () => {
  ...
  physical.route = '/shows/nextup';
  assert.equal(summarizeNextUp([physical], [frame], checksum)[0].actual_client_response_bound, true);
});
