#!/usr/bin/env node
/** Continue the owned zero-history observation through the visible Suggestions tabs. */
...
export const ROOT = WORK + '/reference-nextup-client-discovery-02';
export const TOOL = WORK + '/nextup-client-discovery-tool-02/revision-01';
export const FILES = ['client-browser-nextup-discovery-02.mjs', 'client-browser-nextup-discovery-runtime.mjs', 'client-browser-session-proof.mjs'];
...
export function validateInput(input) {
  need(exact(input, ['kind', 'version', 'runId', 'root', 'execution', 'release', 'sourceClosure', 'existingDeviceIds', 'libraries', 'budgets', 'continuation']));
  need(input.kind === 'nextup-reference-client-discovery-input' && input.version === 1 &&
    input.runId === 'nextup-reference-client-discovery-02' && input.root === ROOT);
  ...
  need(['before', 'after'].every(phase => !input.existingDeviceIds.includes('goby-nextup-client-discovery-02-' + phase)),
  ...
  need(exact(input.continuation, ['priorReport', 'priorIndependentTerminal', 'priorElapsedMilliseconds', 'totalMaximumSeconds']) &&
    descriptor(input.continuation.priorReport) && input.continuation.priorReport.path === WORK + '/reference-nextup-client-discovery-01/private/report.json' &&
    descriptor(input.continuation.priorIndependentTerminal) && input.continuation.priorElapsedMilliseconds === 10586 && input.continuation.totalMaximumSeconds === 1200);
  need(same(input.budgets, { maximumSeconds: 1189, normalSeconds: 829, maximumPlaybackAttempts: 2,
    permittedPlaybackAttempts: 0, recorderRequests: 34, recorderBodyBytes: 2097152, browserNavigationActions: 5 }));
  need(input.budgets.maximumSeconds === Math.floor((input.continuation.totalMaximumSeconds * 1000 - input.continuation.priorElapsedMilliseconds) / 1000) &&
    input.budgets.maximumSeconds - input.budgets.normalSeconds === 360, 'nextup_continuation_budget_changed');
  return input;
}

export function suggestionsSelection(before, after) {
  const tokens = value => new Set((value.class_name ?? '').split(/\s+/).filter(Boolean));
  const beforeShows = tokens(before.shows), afterShows = tokens(after.shows),
    beforeSuggestions = tokens(before.suggestions), afterSuggestions = tokens(after.suggestions);
  const transferred = [...beforeShows].filter(value => !afterShows.has(value) && !beforeSuggestions.has(value) && afterSuggestions.has(value) &&
    /(?:^|[-_])(?:active|selected|current)$/i.test(value));
  const accessible = value => value.aria_selected === 'true' || value.aria_pressed === 'true' || ['true', 'page'].includes(value.aria_current);
  const marked = value => accessible(value) || [...tokens(value)].some(token => /(?:^|[-_])(?:active|selected|current)$/i.test(token));
  const explicit = accessible(after.suggestions), beforeMarked = marked(before.suggestions), competingMarked = marked(after.shows);
  return { selected: !beforeMarked && !competingMarked && (explicit || transferred.length > 0), explicit_accessibility_marker: explicit,
    before_suggestions_explicitly_selected: beforeMarked, after_shows_explicitly_selected: competingMarked,
    transferred_selection_classes: transferred, before, after };
}
...
export function summarizeNextUp(physical, frames, tokenHash) {
  const global = physical.filter(value => typeof value.route === 'string' && /^\/Shows\/NextUp\/?$/i.test(value.route) &&
  ...
  return global.map(value => {
    ...
    return { physical_id: value.id, frame_id: candidates.length === 1 && samePhysical.length === 1 ? candidates[0].id : null,
      ...
      request_start_elapsed_ms: value.start_elapsed_ms,
      frame_finished_elapsed_ms: candidates.length === 1 ? candidates[0].finished_elapsed_ms : null,
...
export async function runDiscovery(filename, inputHash) {
  ...
  const prior = (await privateFile(input.continuation.priorReport.path, input.continuation.priorReport.sha256)).value;
  await privateFile(input.continuation.priorIndependentTerminal.path, input.continuation.priorIndependentTerminal.sha256);
  need(prior.run_id === 'nextup-reference-client-discovery-01' && prior.outcome === 'actual_client_global_request_unobserved' &&
    prior.failure === null && prior.playback_attempts === 0 && prior.recorder_requests === 34 &&
    prior.elapsed_ms === input.continuation.priorElapsedMilliseconds && prior.state?.comparison?.preserved === true &&
    ['before_recorder', 'after_recorder'].every(key => prior.cleanup?.[key]?.exact_token_rejected === true) &&
    prior.cleanup?.browser?.closed === true && prior.cleanup.browser.ui_logout_status === 204 && prior.cleanup.browser.exact_token_status === 401 &&
    prior.cleanup.browser.login_view_visible === true && same(prior.cleanup.browser.cleanup_failures, []), 'nextup_prior_client_not_closed');
  ...
  const maximumMilliseconds = input.budgets.maximumSeconds * 1000, normalMilliseconds = input.budgets.normalSeconds * 1000;
  const report = { kind: 'nextup-reference-client-discovery-report', version: 1, run_id: input.runId,
    ...
    phases: [], state: {}, browser: {}, visible: [], nextup: [], cleanup: {}, failure: null, navigation_actions: 0, suggestions: [],
    continuation: { prior_report_sha256: input.continuation.priorReport.sha256,
      prior_independent_terminal_sha256: input.continuation.priorIndependentTerminal.sha256,
      prior_elapsed_ms: input.continuation.priorElapsedMilliseconds, total_maximum_seconds: input.continuation.totalMaximumSeconds,
      this_scope_maximum_seconds: input.budgets.maximumSeconds, this_scope_normal_seconds: input.budgets.normalSeconds },
    evidence_boundary: { initial_episode_history: 'verified_zero_after_client01_cleanup', playback_attempts: 0,
      establishes: 'Actual original-client parameters and the visible TV Suggestions tabs in zero history.',
  ...
  async function pin(cleanup = false) {
    need(elapsed() <= maximumMilliseconds && (cleanup || elapsed() < normalMilliseconds), 'nextup_time_budget_exhausted');
  ...
  async function call(label, method, route, body, token, device, expectedStatus, cleanup = false, captureLogin = null) {
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      absoluteTimer = setTimeout(() => request.destroy(new Error('nextup_http_absolute_deadline')),
        Math.max(1, Math.min(15000, (cleanup ? maximumMilliseconds : normalMilliseconds) - elapsed())));
  ...
  async function statePhase(name) {
    ...
    const device = 'goby-nextup-client-discovery-02-' + name;
  ...
  async function visible(label) {
    ...
    const view = await actor.page.locator('body').evaluate(element => {
      ...
      const controls = [...element.querySelectorAll('a,button,[role="button"],[role="tab"]')].filter(shown).slice(0, 1000).map(node => ({
        ...
        action: node.getAttribute('data-action'), href: node.getAttribute('href'), class_name: node.getAttribute('class'),
        aria_selected: node.getAttribute('aria-selected'), aria_pressed: node.getAttribute('aria-pressed'),
        aria_current: node.getAttribute('aria-current'), tab_index: node.tabIndex, disabled: node.disabled === true }));
  ...
  async function navigateLibrary(symbol) {
    ...
    need(report.navigation_actions < input.budgets.browserNavigationActions, 'nextup_navigation_budget_exhausted');
    const actionOrdinal = ++report.navigation_actions;
    ...
    await save('navigation-' + symbol + '-intent.json', { action: 'click_visible_library_title', symbol, viewId: library.viewId,
      action_ordinal: actionOrdinal, candidateCount: indices.length, selectedIndex: indices[0], elapsed_ms: elapsed() });
  ...
  async function readTabState() {
    const result = {};
    for (const name of ['Shows', 'Suggestions']) {
      const control = actor.page.getByRole('button', { name, exact: true }).filter({ visible: true });
      need(await control.count() === 1, 'nextup_suggestions_control_ambiguous');
      result[name.toLowerCase()] = await control.evaluate(node => {
        const rect = node.getBoundingClientRect();
        return { tag: node.tagName.toLowerCase(), text: (node.innerText || '').trim(), role: node.getAttribute('role'),
          class_name: node.getAttribute('class'), aria_selected: node.getAttribute('aria-selected'),
          aria_pressed: node.getAttribute('aria-pressed'), aria_current: node.getAttribute('aria-current'),
          tab_index: node.tabIndex, disabled: node.disabled === true, data_index: node.getAttribute('data-index'),
          bounds: { x: rect.x, y: rect.y, width: rect.width, height: rect.height } };
      });
      need(result[name.toLowerCase()].tag === 'button' && result[name.toLowerCase()].text === name &&
        result[name.toLowerCase()].disabled === false, 'nextup_suggestions_control_identity_changed');
    }
    return result;
  }
  async function selectSuggestions(symbol) {
    const before = await readTabState(), library = input.libraries[symbol];
    need(report.navigation_actions < input.budgets.browserNavigationActions, 'nextup_navigation_budget_exhausted');
    const actionOrdinal = ++report.navigation_actions, phaseName = 'suggestions_' + symbol.toLowerCase();
    await phase(phaseName); actor.setPhase(phaseName);
    const stamp = actor.stamp();
    const windowMilliseconds = Math.max(0, Math.min(20000, normalMilliseconds - elapsed())),
      deadline = elapsed() + windowMilliseconds, window = { start_actor_ms: stamp.elapsed_ms,
        deadline_actor_ms: stamp.elapsed_ms + windowMilliseconds, maximum_wait_ms: windowMilliseconds };
    await save('navigation-' + symbol + '-suggestions-intent.json', { action: 'click_visible_suggestions_button', symbol,
      view_id: library.viewId, action_ordinal: actionOrdinal, phase: phaseName, document_id: actor.documentID,
      route: actor.page.url(), before, transport_sequence: stamp.sequence, window, elapsed_ms: elapsed() });
    await actor.page.getByRole('button', { name: 'Suggestions', exact: true }).filter({ visible: true }).click({ timeout: 10000 });
    let after = before, selection = suggestionsSelection(before, before), responses = [];
    while (elapsed() < deadline) {
      let timer;
      try {
        after = await Promise.race([readTabState(), new Promise((_, reject) => {
          timer = setTimeout(() => reject(new Error('nextup_suggestions_window_elapsed')), Math.max(1, deadline - elapsed()));
        })]);
      } catch (error) { if (error.message === 'nextup_suggestions_window_elapsed') break; throw error; }
      finally { clearTimeout(timer); }
      if (elapsed() > deadline) break;
      selection = suggestionsSelection(before, after);
      responses = summarizeNextUp([...physical.values()], [...frames.values()], actor.report.token_sha256)
        .filter(row => row.navigation_cause === phaseName && row.actual_client_response_bound &&
          row.request_start_elapsed_ms >= window.start_actor_ms && row.finished_elapsed_ms <= window.deadline_actor_ms &&
          row.frame_finished_elapsed_ms <= window.deadline_actor_ms);
      if (selection.selected && responses.length) break;
      await actor.page.waitForTimeout(Math.max(1, Math.min(100, deadline - elapsed())));
    }
    await visible('suggestions-' + symbol.toLowerCase());
    const route = new URL(actor.page.url()), hash = route.hash.replace(/^#!/, '').replace(/^#/, ''),
      query = new URLSearchParams(hash.includes('?') ? hash.slice(hash.indexOf('?') + 1) : '');
    need([...query].some(([key, value]) => ['parentid', 'topparentid', 'id', 'viewid'].includes(key.toLowerCase()) && value === library.viewId),
      'nextup_suggestions_library_scope_changed');
    const receipt = await save('suggestions-' + symbol + '-result.json', { symbol, action_ordinal: actionOrdinal, phase: phaseName,
      document_id: actor.documentID, route: actor.page.url(), selection, transport_sequence_before: stamp.sequence,
      transport_sequence_after: actor.sequence, window, completed_global_responses: responses, elapsed_ms: elapsed() });
    report.suggestions.push({ symbol, view_id: library.viewId, action_ordinal: actionOrdinal, selection,
      phase: phaseName, window,
      response_physical_ids: responses.map(row => row.physical_id), response_frame_ids: responses.map(row => row.frame_id),
      completed_global_response_count: responses.length, result_receipt_sha256: receipt.sha256 });
    need(selection.selected, 'nextup_suggestions_selection_unconfirmed');
  }
  ...
  try {
    ...
    actor = await createReferenceBrowserActor({ account: { id: REFERENCE_VIEWER, username: credential.username, password: credential.password },
      ...
      existingDeviceIDs: [...input.existingDeviceIds, 'goby-nextup-client-discovery-02-before', 'goby-nextup-client-discovery-02-after'],
    ...
    await selectSuggestions('LA');
    need(report.navigation_actions < input.budgets.browserNavigationActions, 'nextup_navigation_budget_exhausted');
    const homeActionOrdinal = ++report.navigation_actions;
    await phase('navigate_home'); actor.setPhase('navigate_home');
    await save('navigation-home-intent.json', { action: 'click_visible_home_link', action_ordinal: homeActionOrdinal, elapsed_ms: elapsed() });
    ...
    await selectSuggestions('LB');
    need(report.navigation_actions === input.budgets.browserNavigationActions, 'nextup_navigation_sequence_incomplete');
    await actor.settled();
  } catch (error) {
    ...
    if (actor) {
      ...
      report.nextup = report.nextup.map(row => {
        const observation = report.suggestions.find(value => value.phase === row.navigation_cause);
        return { ...row, within_suggestions_window: Boolean(observation && row.request_start_elapsed_ms >= observation.window.start_actor_ms &&
          row.finished_elapsed_ms <= observation.window.deadline_actor_ms && row.frame_finished_elapsed_ms <= observation.window.deadline_actor_ms) };
      });
    ...
    const bound = report.nextup.filter(value => value.actual_client_response_bound && value.within_suggestions_window && Array.isArray(value.ordered_ids));
    ...
    report.continuation.total_elapsed_ms = input.continuation.priorElapsedMilliseconds + report.elapsed_ms;
    if (report.elapsed_ms > maximumMilliseconds || report.continuation.total_elapsed_ms > input.continuation.totalMaximumSeconds * 1000) {
      report.failure ??= 'nextup_time_budget_exhausted'; report.outcome = 'failed';
    }
