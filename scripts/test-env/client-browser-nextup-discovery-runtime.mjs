/** A fixed-P NextUp discovery transport with passive, bounded public evidence. */
...
export const REFERENCE_VIEWER = 'b886fd2f2c6241db9b973c8f038f9105';
export const REFERENCE_SERVER = 'f56dec8ff7414847873064c4be9fba74';
export const NEXTUP_DISCOVERY_SCOPE = Object.freeze({ format: 1, origin: REFERENCE_ORIGIN,
  viewerId: REFERENCE_VIEWER, serverId: REFERENCE_SERVER, slot: 'P',
  ownedItemIds: Object.freeze(['137', '138', '139', '140', '141', '142', '143', '144', '145', '146', '147', '148']),
  viewIds: Object.freeze(['133', '135']), publicResponseRoute: '/Shows/NextUp', playbackAllowed: false,
...
const TOKEN_KEYS = new Set(['token', 'accesstoken', 'access_token', 'api_key', 'apikey', 'x-emby-token', 'x-mediabrowser-token']);
const SECRET_KEYS = new Set([...TOKEN_KEYS, 'password', 'pw', 'passwordhash', 'authorization', 'x-emby-authorization', 'username']);
const CREDENTIAL_FIELDS = new Set([...SECRET_KEYS].map(value => value.replace(/[-_]/g, '')));
const credentialField = value => CREDENTIAL_FIELDS.has(value.toLowerCase().replace(/[-_]/g, ''));
const OWNED_CATALOG_IDS = new Set([...NEXTUP_DISCOVERY_SCOPE.ownedItemIds, ...NEXTUP_DISCOVERY_SCOPE.viewIds]);
...
export function classifyReferenceRequest(raw, method, resourceType = '') {
  try {
    ...
    if (resourceType === 'media' || /^\/(?:Videos|Audio)(?:\/|$)/i.test(route) ||
      /^\/Items\/[^/]+\/(?:Download|File)(?:\/|$)/i.test(route) ||
...
function assertPublicJSON(value, secrets) {
  ...
  function visit(current, depth) {
    ...
    if (Array.isArray(current)) current.forEach(item => visit(item, depth + 1));
      ...
      guard(!credentialField(key) || item === null || item === '', 'reference_message_contains_credential_field');
      visit(item, depth + 1);
    }
  }
  visit(value, 0);
...
function nextUpRoute(route) { return /^\/Shows\/NextUp\/?$/i.test(route); }

function catalogRoute(route) {
  const own = `/Users/${REFERENCE_VIEWER}`;
  const selected = route.replace(/\/$/, '');
  if ([`${own}/Views`, '/Views', `${own}/Items`, '/Items'].some(value => value.toLowerCase() === selected.toLowerCase())) return true;
  const item = new RegExp(`^(?:/Users/${REFERENCE_VIEWER})?/Items/([^/]+)$`, 'i').exec(selected);
  return Boolean(item && OWNED_CATALOG_IDS.has(item[1]));
}

function captureRoute(route, queryEntries, method) {
  if (method !== 'GET') return false;
  if (nextUpRoute(route)) return true;
  const own = `/Users/${REFERENCE_VIEWER}`;
  const selected = route.replace(/\/$/, '');
  if ([`${own}/Views`, '/Views'].some(value => value.toLowerCase() === selected.toLowerCase())) return true;
  const item = new RegExp(`^(?:/Users/${REFERENCE_VIEWER})?/Items/([^/]+)$`, 'i').exec(selected);
  if (item) return OWNED_CATALOG_IDS.has(item[1]);
  if (![`${own}/Items`, '/Items'].some(value => value.toLowerCase() === selected.toLowerCase())) return false;
  const entries = queryEntries.map(([key, value]) => [key.toLowerCase(), value]);
  const parent = entries.filter(([key]) => key === 'parentid').map(([, value]) => value);
  const ids = entries.filter(([key]) => key === 'ids').map(([, value]) => value);
  return parent.length === 1 && OWNED_CATALOG_IDS.has(parent[0]) ||
    ids.length === 1 && ids[0].length > 0 && ids[0].split(',').every(value => OWNED_CATALOG_IDS.has(value));
}
...
export function referenceRequestRecord(raw, method, headers = {}, secrets = []) {
  const { url, route } = normalizedURL(raw, true), query = {}, queryEntries = [], hidden = [];
  const authority = referenceAuthority(raw, headers);
  if (authority) secrets = [...new Set([...secrets, authority.token])];
  const nextUp = nextUpRoute(route), catalog = catalogRoute(route);
  ...
  for (const [key, value] of url.searchParams) {
    ...
    if (credentialField(key)) continue;
    guard(!hasSecret(key, secrets), 'reference_query_name_contains_secret');
    if (nextUp || catalog && PUBLIC_QUERY.has(key.toLowerCase())) {
      guard(!hasSecret(value, secrets), 'reference_public_query_rejected');
      queryEntries.push([key, value]);
      // This compatibility view keeps the first value; exact_public_query is authoritative.
  ...
  return assertPublicJSON({ method: safeMethod(method), kind: classifyReferenceRequest(raw, method).kind, route, query,
    exact_public_query: queryEntries, exact_public_query_semantics: 'Ordered decoded public URLSearchParams entries; duplicates and empty values retained',
    hidden_query: hidden, shape_sha256: sha(route + '\n' + JSON.stringify(nextUp ? queryEntries : sorted) + '\n' + JSON.stringify(hidden)),
    request_sha256: sha(method + '\n' + raw), token_sha256: authority?.token_sha256 ?? null,
    capture: captureRoute(route, queryEntries, method) }, secrets);
}

/** Public item identity and UserData are retained; response entity bytes stay intact. */
...
export function referenceResponseProjection(bytes, secrets = [], encoding = null) {
  ...
  function item(value) {
    ...
    if (Object.hasOwn(value, 'UserData')) {
      guard(value.UserData === null || record(value.UserData), 'reference_userdata_projection_shape');
      projected.UserData = value.UserData;
    }
  ...
  return assertPublicJSON({ body_sha256: sha(bytes), bytes: bytes.length, decoded_bytes: decoded.length, decoded_body_sha256: sha(decoded),
    content_encoding: encoding ?? 'identity', json: projected, projection: 'public_item_identity_and_userdata_fields' }, secrets);
}

/** Only the allowed NextUp response body reaches this bounded frame digest. */
export function referenceFrameBodyEvidence(bytes) {
  guard(Buffer.isBuffer(bytes) && bytes.length <= L.captureBytes, 'reference_frame_body_limit');
  return { decoded_body_sha256: sha(bytes), decoded_body_bytes: bytes.length };
}

/** Raw response bytes may reach only a named private synchronous sink. */
export function deliverReferencePrivateResponse(observer, callbackName, evidence, encoding, body) {
  guard(record(observer) && ['onPrivateResponse', 'onPrivateLoginResponse'].includes(callbackName),
    'reference_private_response_callback_rejected');
  const callback = observer[callbackName];
  if (callback === undefined) return;
  guard(typeof callback === 'function' && record(evidence) && Buffer.isBuffer(body) && body.length <= L.captureBytes,
    'reference_private_response_scope_rejected');
  guard(callbackName === 'onPrivateResponse'
    ? evidence.method === 'GET' && nextUpRoute(evidence.route)
    : evidence.kind === 'login' && evidence.method === 'POST' && /^\/Users\/AuthenticateByName\/?$/i.test(evidence.route),
  'reference_private_response_scope_rejected');
  const result = callback.call(observer, { request: structuredClone(evidence), encoding: encoding ?? 'identity', body_base64: body.toString('base64') });
  if (result && typeof result.then === 'function') void Promise.resolve(result).catch(() => {});
  guard(result === undefined, 'reference_private_observer_must_be_synchronous');
}
...
export async function createReferenceBrowserActor(options = {}) {
  ...
  const { account, pin, observer = {}, onLogin, report = {}, existingDeviceIDs, beforePrivateMutation } = options;
  guard(record(account) && account.id === REFERENCE_VIEWER && typeof account.username === 'string' &&
    ...
    (beforePrivateMutation === undefined || typeof beforePrivateMutation === 'function') &&
    (observer.onPrivateResponse === undefined || typeof observer.onPrivateResponse === 'function') &&
    (observer.onPrivateLoginResponse === undefined || typeof observer.onPrivateLoginResponse === 'function') &&
  ...
  Object.assign(report, { format: 1, origin: REFERENCE_ORIGIN, user_id: REFERENCE_VIEWER, server_id: REFERENCE_SERVER,
    discovery_scope: NEXTUP_DISCOVERY_SCOPE,
  ...
  const pageRoute = () => {
    ...
    try {
      ...
      if ([...value.searchParams].some(([key]) => credentialField(key))) return null;
  ...
  /** This private synchronous sink is never routed through public observers. */
  function notifyPrivateResponse(evidence, encoding, body) {
    try {
      deliverReferencePrivateResponse(observer, 'onPrivateResponse', evidence, encoding, body);
    } catch { report.observer_errors += 1; fail('private_response_observer_failed'); }
  }
  /** Preserve a private login response before trusting any returned principal. */
  function notifyPrivateLoginResponse(evidence, encoding, body) {
    try {
      deliverReferencePrivateResponse(observer, 'onPrivateLoginResponse', evidence, encoding, body);
    } catch { report.observer_errors += 1; fail('private_login_response_observer_failed'); }
  }
  /** Persist each private write intent before the dispatcher may contact the target. */
  async function journalPrivateMutation(evidence, bytes) {
    if (beforePrivateMutation === undefined || !['login', 'capabilities', 'logout'].includes(evidence.kind)) return;
    evidence.private_mutation_journal = { attempted: true, completed: false };
    try {
      await bounded(Promise.resolve().then(() => beforePrivateMutation({ request: structuredClone(evidence),
        body_base64: bytes.toString('base64') })), 15000, 'reference_private_mutation_journal_timeout');
      evidence.private_mutation_journal.completed = true;
      evidence.private_mutation_journal.completed_elapsed_ms = elapsed();
    } catch {
      fail('private_mutation_journal_failed');
      throw new Error('reference_private_mutation_journal_failed');
    }
  }
  ...
  async function captureLogin(bytes, encoding) {
    ...
    pendingLoginProof = { token_sha256: sha(value.AccessToken), session_id: value.SessionInfo.Id, user_id: value.User.Id,
      ...
      created_at_source: 'SessionInfo.LastActivityDate', kind: 'emby', slot: NEXTUP_DISCOVERY_SCOPE.slot, frame_login_finished: false,
  ...
  async function forwardHTTP(request, response, requestIndex) {
    ...
    try {
      ...
      evidence.request_body_sha256 = sha(bytes); evidence.request_bytes = requestBytes;
      await journalPrivateMutation(evidence, bytes);
      guard(!finished && trafficAllowed(plan.kind), 'reference_private_mutation_dispatch_rejected');
        ...
        void dispatchReferenceHTTP({ plan, bytes, account, evidence,
          ...
          void (async () => {
            try {
              ...
              if (plan.kind === 'login') {
                guard(responseBytes <= L.captureBytes, 'reference_private_login_response_limit');
                notifyPrivateLoginResponse(evidence, reply.contentEncoding, body);
                guard(trafficAllowed(plan.kind) && !finished, 'reference_private_login_response_observer_rejected');
              }
              if (plan.kind === 'read' && plan.method === 'GET' && nextUpRoute(plan.route)) {
                guard(responseBytes <= L.captureBytes, 'reference_private_response_limit');
                Object.assign(evidence, { sequence: responseStamp.sequence, elapsed_ms: responseStamp.elapsed_ms,
                  response_body_sha256: sha(body), response_bytes: responseBytes });
                notifyPrivateResponse(evidence, reply.contentEncoding, body);
                guard(trafficAllowed(plan.kind) && !finished, 'reference_private_response_observer_rejected');
              }
              if (capture) {
  ...
  function observeContextRequest(request) {
    ...
    const operation = (async () => {
      if (external) {
        frameHTTP.push(evidence); notify('onFrameRequest', evidence, true);
      ...
      // Resolve all authority carriers before publishing exact public query values.
      Object.assign(evidence, referenceRequestRecord(raw, request.method(), headers, secrets));
  ...
  function observeContextResponse(response) {
    ...
    track((async () => {
      ...
      if (nextUpRoute(evidence.route) && evidence.method === 'GET' && evidence.status === 200 && evidence.main_frame === true &&
        evidence.sourceworker === false && evidence.from_service_worker === false) {
        guard(evidence.content_type === 'application/json');
        const bytes = await bounded(response.body(), L.httpMs, 'reference_frame_body_timeout');
        evidence.frame_body = referenceFrameBodyEvidence(bytes);
      }
  ...
  try {
    ...
    report.playwright_version = require(`${PLAYWRIGHT}/package.json`).version;
