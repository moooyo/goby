...
function validHopOperator(hop) {
  ...
  if (hop.from_schema === 25 && hop.to_schema === 25) return hop.method === LEGACY_HOP_METHOD &&
    hop.operator_path === LEGACY_OPERATOR_PATH && hop.operator_sha256 === LEGACY_OPERATOR_SHA;
  return [25, 26].includes(hop.from_schema) && hop.to_schema === 26 && hop.method === SCHEMA_26_HOP_METHOD &&
    digest(SCHEMA_26_OPERATOR_SHA) && hop.operator_path === `${ROOT}/prepare-client-fixture.py` &&
    hop.operator_sha256 === SCHEMA_26_OPERATOR_SHA;
}
...
function pureLoaderCore(loaderSource, unfrozenOperator = false) {
  ...
  if (unfrozenOperator) {
    const declaration = `const SCHEMA_26_OPERATOR_SHA = '${SCHEMA_26_OPERATOR_SHA}';`;
    requireThat(source.split(declaration).length === 2);
    source = source.replace(declaration, 'const SCHEMA_26_OPERATOR_SHA = null;');
  }
  let effects = 0;
  ...
  new vm.Script(source + '\nglobalThis.__musicCore = Object.freeze({schemaBinding, musicPreservationBinding, upgradeOperatorBinding});',
    { filename: 'client-browser-goby-fixture.pure.mjs' }).runInContext(context, { timeout: 1000 });
  requireThat(effects === 0);
  return {
    invoke(name, ...args) {
      requireThat(['schemaBinding', 'musicPreservationBinding', 'upgradeOperatorBinding'].includes(name));
...
function syntheticSource(schema) {
  return { marker: `goby-client-schema${schema}-source-m3e-v1`, schema,
    source: `${ROOT}/source-attempt-${schema === 25 ? 9001 : 9002}`,
    source_manifest_sha256: syntheticHash(`manifest-${schema}`),
    catalog_sha256: schema === 25 ? CATALOG_25_SHA : CATALOG_26_SHA,
    [`migration_${schema}_sha256`]: schema === 25 ? MIGRATION_25_SHA : MIGRATION_26_SHA };
}

function syntheticState(schema, fromSchema = 25) {
  ...
  if (schema === 26) {
    ...
    artifacts.operator = { path: `${ROOT}/prepare-client-fixture.py`, sha256: SCHEMA_26_OPERATOR_SHA };
...
function syntheticSummary(schema) {
  ...
  const summary = { schema, database_sha256: syntheticHash(`database-${schema}`), recovery_sha256: syntheticHash('recovery'),
    runtime_sha256: syntheticHash('runtime'), browser_sha256: syntheticHash('browser'), table_count: schema === 25 ? 30 : 33,
...
function syntheticHop(from, to) {
  const before = syntheticState(from, from === 25 ? 24 : 25), state = syntheticState(to, from);
...
export function runPureMusicLineageGuards(loaderSource) {
  ...
  for (const schema of [23, 27, '26', null]) schemaNegative(`unsupported_state_schema_${schema}`, state => { state.schema = schema; });
  ...
  for (const [from, to] of [[24, 25], [24, 26], [26, 25], [26, 27]]) test(`operator_unsupported_${from}_to_${to}`, () => {
  ...
  try {
    ...
    try {
      ...
      if (chainMode) {
        ...
        check(lineage.method === (lineage.hops.every(hop => hop?.from_schema === 25 && hop?.to_schema === 25)
          ? LEGACY_CHAIN_METHOD : MIXED_CHAIN_METHOD));
      ...
      check(lineage.from_schema === 25 && [25, 26].includes(lineage.to_schema) && scan.schema === 25 && evidence.schema === lineage.to_schema);
