...
INPUT = W / 'nextup-client-discovery-inputs-01/input.json'
OUTPUT = W / 'reference-nextup-client-discovery-01'
UNIT = 'goby-nextup-client-discovery-01.service'
UNIT_FILE = Path('/run/systemd/system') / UNIT
PREVIOUS = W / 'reference-nextup-global-matrix-execution-07/preservation-after.json'
PREVIOUS_SHA = '86b172a43e89f0dd3433831ccb3d5ec21c5a4a04c15c8b00c5bf278b6a906687'
...
def main():
    ...
    require(len(previous['roots']) == previous['root_count'] == 186, 'The accepted baseline root count differs.')
    ...
    roots = sorted(previous['roots']) + [str(W / name) for name in (
        'reference-nextup-global-matrix-execution-07', 'nextup-global-reference-operator-runs-07',
        'nextup-global-reference-runs-05', 'matrix07-publication-review-01',
        'nextup-client-discovery-tool-01', 'nextup-client-discovery-inputs-01')]
    require(len(roots) == len(set(roots)) == 192, 'The client preservation root inventory differs.')
    trees = {}
    for name in roots:
        root = Path(name)
    ...
    module = types.ModuleType('nextup_client_owned_goby_snapshot')
    ...
    baseline = module.precise_json(read(previous['goby_snapshot']['path'], previous['goby_snapshot']['sha256']))
    stable = []
    ...
    main_files = {name: info(Path(name), expected) for name, expected in previous['main_files'].items()}
    require(main_files == previous['main_files'] and info(UNIT_FILE) == unit_file, 'A protected file changed.')
    ...
    record = {'schemaVersion': 1, 'kind': 'nextup-client-preservation', 'phase': phase,
              ...
              'historical186RootsEqual': True, 'completeGobyStateEqualExceptCaptureTime': True,
