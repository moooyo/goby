#!/usr/bin/env python3
"""Guard source49/schema28 full disposal with two reads and no live effects.
...
EXPECTED_RUNNER = PurePosixPath('/opt/goby-test/exec-work-m3e/source-attempt-49/scripts/test-env/run-client-backup-tests.py')
...
EXPECTED_LOG_PATH = ast.parse("OUTPUT / 'go.log'", mode='eval').body
...
EXPECTED_ABSENT_BINARY = ast.parse("not m.present(OUTPUT / 'tmp/goby-linux-amd64')", mode='eval').body
...
UNSAFE_CALL_TEMPLATES = [ast.parse(source).body[0] for source in (
    ...
    'runner.run_all()', 'runner.start_unit()', 'm.Runner(args).run_all()',
...
def validate_log_read_limits(tree):
    ...
    for node in memory_walk(tree):
        ...
        if path == ast.dump(EXPECTED_LOG_PATH):
            require(len(limits) == 1 and isinstance(limits[0], ast.Name) and limits[0].id == 'LOG_BYTES',
                    'The full Go log fell back to the eight-MiB default read limit.')
            direct.append(node)
        elif path == ast.dump(EXPECTED_PINNED_PATH):
            require(len(limits) == 1 and ast.dump(limits[0]) == ast.dump(EXPECTED_PINNED_LOG_LIMIT),
                    'A repeated evidence pin no longer reads the complete Go log.')
            pinned.append(node)
        elif root.lineno <= node.lineno <= root.end_lineno:
            require(len(limits) == 1 and ast.dump(limits[0]) == ast.dump(EXPECTED_TREE_LIMIT),
                    'The historical tree reader cannot witness the complete bounded log.')
            historical.append(node)
    require(len(direct) == 1 and len(pinned) == 2 and len(historical) == 1,
            'The exact full-log observation and repeated pin locations changed.')
...
def receipt_fixture():
    return {'marker': 'goby-client-backup-pair-m3e-v1', 'run_id': OP.RUN, 'tag': OP.TAG, 'unit': OP.UNIT,
        ...
        'schema': 28, 'catalog_sha256': OP.CATALOG_SHA, 'mode': 'full', 'phase': 'retained',
...
def full_terminal_fixture():
    state = {'MainPID': '0', 'Result': 'exit-code', 'ExecMainStatus': '1', 'ControlGroup': '', 'SubState': 'failed'}
    return {'marker': 'goby-source49-schema28-full-failed-terminal-v1', 'unit': OP.OUTER_UNIT, 'inner_unit': OP.UNIT,
        'state': dict(state, InvocationID=OP.UNIT_PINS[OP.OUTER_UNIT]['InvocationID'],
                      Description=OP.UNIT_PINS[OP.OUTER_UNIT]['Description']),
        'inner_state': dict(state, InvocationID=OP.UNIT_PINS[OP.UNIT]['InvocationID'], Description=OP.TAG),
        'report_sha256': OP.REPORT_SHA, 'go_log_sha256': OP.LOG_SHA, 'go_log_bytes': OP.LOG_BYTES,
        'counts': {'pass': 2105, 'fail': 2}, 'failed_tests': list(OP.FAILED_TESTS),
        'packages': [{'package': name, 'status': status} for name, status in OP.PACKAGE_RESULTS],
        'recursive_cgroup_empty': True, 'pair_retained': True, 'recoverydb_executed': False,
        'linux_build_artifact_exists': False}


def full_log_events():
    events = [{'Action': 'pass', 'Package': 'github.com/moooyo/goby/cmd/goby',
               'Test': 'TestSyntheticFullPass' + str(index)} for index in range(2105)]
    events += [{'Action': 'fail', 'Package': 'github.com/moooyo/goby/internal/recovery', 'Test': name}
               for name in OP.FAILED_TESTS]
    events += [{'Action': status, 'Package': name} for name, status in OP.PACKAGE_RESULTS]
    return events
...
class DisposalGuards(unittest.TestCase):
    def test_exact_run_source_runner_and_pair_pins(self):
        self.assertEqual(OP.RUN, '20260912_042841_89ef5954034a')
        self.assertEqual(str(OP.SOURCE), '/opt/goby-test/exec-work-m3e/source-attempt-49')
        self.assertEqual(str(OP.OUTPUT), '/opt/goby-test/exec-work-m3e/client-backup-run-20260912_042841_89ef5954034a')
        self.assertEqual(str(OP.EXECUTION), '/opt/goby-test/exec-work-m3e/storage-binding-schema28-full-execution-01')
        self.assertEqual(OP.MANIFEST_SHA, 'a22a89b460a1bf5d4033336ea57fd7a8e69975c01bf883ad54fccf1cd5c6ee3a')
        ...
        self.assertEqual(OP.PAIRS, [
            {'name': 'goby_backup_m3e_source', 'role_oid': 16410704, 'database_oid': 16410705},
            {'name': 'goby_backup_m3e_target', 'role_oid': 16410706, 'database_oid': 16410707}])
        self.assertEqual(OP.PUBLIC_OIDS, {'goby_backup_m3e_source': 2200, 'goby_backup_m3e_target': 17186575})
        ...
        self.assertEqual(OP.OUTPUT_IDENTITY, {'device': 2049, 'inode': 2504671})
        self.assertEqual(OP.RECEIPT_SHA, 'b5a891eb63d1d7adae6e841a4162b408bf9e1e183b8ddc0773404c38e9dfc684')
        self.assertEqual(OP.REPORT_SHA, '4d61ae8737438ccf03a11b43983898cb23a60a0f3ec2b64781b7ea1fc49f03be')
        self.assertEqual(OP.LOG_SHA, '138e48ed7d890e3bd26203a31fdec8220c8a49d7d559d4846f96536d5a5d0f5c')
        self.assertEqual(OP.LOG_BYTES, 9406631)
        self.assertEqual(OP.TERMINAL_SHA, 'a6dde10cb27f1408a28faaaf12c7d0904f6fa72758bbaf68fe8777a808db92c5')
        self.assertEqual(OP.TREE_SHA, {
            str(OP.SOURCE): 'a18e3df758f17774cdebb1fe07ba2bbe1693180358cb833fac01b29fc9478c89',
            str(OP.OUTPUT): '4eb53dc7ba188b3342a898a30a140c17cb903658394f2c68f8ea2b8f1ee08ea8',
            str(OP.EXECUTION): 'c8431a96d3291935732c7f96d8ad16786a0ae1245963e292fd97723ffc7d7f0e'})

    def test_full_launch_rejects_targeted_selectors_and_wrong_input_types(self):
        expected = {'source': OP.SOURCE, 'manifest_sha256': OP.MANIFEST_SHA, 'schema': 28,
                    'mode': 'full', 'run': '', 'package': []}
        m = SimpleNamespace(require=require)
        OP.validate_full_launch(m, SimpleNamespace(**expected))
        mutations = [('source', OP.WORK / 'source-attempt-47'), ('manifest_sha256', '0' * 64),
            ('schema', 27), ('schema', 28.0), ('schema', True), ('mode', 'targeted'),
            ('run', '^TestRootBinding'), ('run', None), ('package', ['./internal/database']),
            ('package', ()), ('package', None)]
        for key, value in mutations:
            with self.subTest(field=key, value=value), self.assertRaises(ValueError):
                OP.validate_full_launch(m, SimpleNamespace(**dict(expected, **{key: value})))

    def test_full_failure_set_and_package_results_are_independently_pinned(self):
        self.assertEqual(OP.FAILED_TESTS, (
            'TestRecoveryManagerSchema23EncryptedArchiveApplyRestartAndRollback',
            'TestRecoveryManagerSchema24EncryptedArchiveApplyRestartAndRollback'))
        passed = ('cmd/goby', 'internal/activity', 'internal/artwork', 'internal/backupformat', 'internal/backuppg',
            ...
            'internal/playback', 'internal/recoverycontrol', 'internal/server', 'internal/settings',
            'internal/storagebinding', 'internal/subtitle', 'internal/tasks', 'internal/transcode')
        expected = [('github.com/moooyo/goby/' + name, 'pass') for name in passed]
        expected.append(('github.com/moooyo/goby/internal/recovery', 'fail'))
        self.assertEqual(OP.PACKAGE_RESULTS, tuple(sorted(expected)))

    def test_complete_full_log_parses_exact_counts_failures_and_packages(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        events = full_log_events()
        events.extend(({'Action': 'output', 'Package': 'github.com/moooyo/goby/internal/recovery', 'Output': 'Synthetic failure detail.'},
                       {'Action': 'fail', 'Package': 'github.com/moooyo/goby/internal/recovery', 'Test': OP.FAILED_TESTS[0] + '/subcase'}))
        summary = OP.full_log_summary(m, b'Synthetic command preface\n' + log_bytes(events))
        self.assertEqual(summary['counts'], {'pass': 2105, 'fail': 2, 'skip': 0})
        self.assertEqual(summary['failed_tests'], list(OP.FAILED_TESTS))
        self.assertEqual(summary['packages'], [{'package': name, 'status': status} for name, status in OP.PACKAGE_RESULTS])
        self.assertIs(summary['unique_top_level'], True)
        self.assertIs(summary['recoverydb_executed'], False)
        self.assertIs(summary['race_warning'], False)
        OP.validate_full_evidence(m, full_terminal_fixture(), summary, False)

    def test_log_summary_preserves_events_beyond_old_eight_mib_limit(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        raw = b'Synthetic bounded preface ' + b'x' * (8 << 20) + b'\n' + log_bytes(full_log_events())
        self.assertGreater(len(raw), 8 << 20)
        summary = OP.full_log_summary(m, raw)
        OP.validate_full_evidence(m, full_terminal_fixture(), summary, False)

    def test_log_parser_rejects_empty_nonbytes_and_malformed_records(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        for raw in (b'', '', None, b'not a JSON event\n', b'{malformed', b'{"Action":"pass"}\n'):
            with self.subTest(raw=raw), self.assertRaises(ValueError):
                OP.full_log_summary(m, raw)
    ...
    def test_full_evidence_rejects_changed_failure_count_names_and_packages(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        for defect in ('missing_pass', 'extra_pass', 'wrong_failure', 'skip', 'duplicate_test',
                       'missing_package', 'duplicate_package', 'wrong_package_status', 'recoverydb', 'race'):
            events = full_log_events()
            suffix = b''
            if defect == 'missing_pass':
                del events[0]
            elif defect == 'extra_pass':
                events.append({'Action': 'pass', 'Package': 'synthetic/extra', 'Test': 'TestExtra'})
            elif defect == 'wrong_failure':
                next(event for event in events if event.get('Action') == 'fail' and event.get('Test'))['Test'] = 'TestUnreviewedFailure'
            elif defect == 'skip':
                ...
                events.pop()
            elif defect == 'duplicate_package':
                events.append(copy.deepcopy(events[-1]))
            elif defect == 'wrong_package_status':
                events[-1]['Action'] = 'fail'
            elif defect == 'recoverydb':
                events.append({'Action': 'start', 'Package': 'github.com/moooyo/goby/internal/recoverydb'})
            else:
                suffix = b'WARNING: DATA RACE\n'
            with self.subTest(defect=defect):
                summary = OP.full_log_summary(m, log_bytes(events) + suffix)
                with self.assertRaises(ValueError):
                    OP.validate_full_evidence(m, full_terminal_fixture(), summary, False)

    def test_full_terminal_rejects_missing_changed_and_truthy_proofs(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        summary = OP.full_log_summary(m, log_bytes(full_log_events()))
        terminal = full_terminal_fixture()
        for key in terminal:
            changed = copy.deepcopy(terminal)
            del changed[key]
            with self.subTest(missing=key), self.assertRaises(ValueError):
                OP.validate_full_evidence(m, changed, summary, False)
        for key, value in [('marker', 'other'), ('unit', OP.UNIT), ('inner_unit', OP.OUTER_UNIT),
                ('report_sha256', '0' * 64), ('go_log_sha256', '0' * 64), ('go_log_bytes', float(OP.LOG_BYTES)),
                ('counts', {'pass': 2105.0, 'fail': 2}), ('failed_tests', list(reversed(OP.FAILED_TESTS))),
                ('packages', []), ('recursive_cgroup_empty', 1), ('pair_retained', 1),
                ('recoverydb_executed', 0), ('linux_build_artifact_exists', 0)]:
            with self.subTest(field=key, value=value), self.assertRaises(ValueError):
                OP.validate_full_evidence(m, dict(terminal, **{key: value}), summary, False)
        for section in ('state', 'inner_state'):
            for key in terminal[section]:
                changed = copy.deepcopy(terminal)
                changed[section][key] = 'changed'
                with self.subTest(section=section, field=key), self.assertRaises(ValueError):
                    OP.validate_full_evidence(m, changed, summary, False)
        for exists in (True, 0, None):
            with self.subTest(binary_exists=exists), self.assertRaises(ValueError):
                OP.validate_full_evidence(m, terminal, summary, exists)

    def test_full_summary_rejects_boolean_counts_and_jointly_changed_terminal_claims(self):
        m = SimpleNamespace(require=require, decode=memory_json_decode)
        summary = OP.full_log_summary(m, log_bytes(full_log_events()))
        for key, value in [('counts', {'pass': 2105.0, 'fail': 2, 'skip': 0}),
                           ('recoverydb_executed', 0), ('race_warning', 0), ('unique_top_level', 1), ('extra', True)]:
            with self.subTest(field=key), self.assertRaises(ValueError):
                OP.validate_full_evidence(m, full_terminal_fixture(), dict(summary, **{key: value}), False)
        terminal = full_terminal_fixture()
        changed = copy.deepcopy(summary)
        changed['failed_tests'] = ['TestOtherOne', 'TestOtherTwo']
        terminal['failed_tests'] = list(changed['failed_tests'])
        with self.assertRaises(ValueError):
            OP.validate_full_evidence(m, terminal, changed, False)
    ...
    def test_retained_receipt_rejects_every_changed_or_missing_field(self):
        ...
        mutations = [('run_id', '20260912_000000_000000000000'), ('source', str(OP.SOURCE) + '-other'),
            ('mode', 'targeted'), ('schema', 27), ('schema', True), ('phase', 'finished'),
    ...
    def test_retained_public_identity_is_distinct_from_recreated_target_namespace(self):
        ...
        self.assertEqual(OP.public_expected(target['name'])['oid'], 17186575)
        self.assertNotEqual(target['public'], snapshot_fixture(target)['public'])
    ...
    def test_failed_report_proves_the_exact_failure_and_restored_hba(self):
        report = {'marker': 'goby-client-backup-pair-m3e-v1', 'status': 'failed', 'run_id': OP.RUN,
            'source': str(OP.SOURCE), 'source_manifest_sha256': OP.MANIFEST_SHA, 'schema': 28, 'mode': 'full',
    ...
    def removal_model(self, index=0, before_sessions='0', raced_sessions='0', nonempty=False):
        ...
        runner = SimpleNamespace(hba_before=b'memory-only-hba', tag=OP.TAG, output=OP.OUTPUT,
            args=SimpleNamespace(source=OP.SOURCE, manifest_sha256=OP.MANIFEST_SHA, schema=28, mode='full', run='', package=[]),
    ...
    def test_full_log_ast_limits_cover_initial_rechecks_summary_and_tree_reader(self):
        locations = validate_log_read_limits(TREE)
        self.assertEqual(len(locations), 4)
        self.assertGreater(OP.LOG_BYTES, 8 << 20)
        self.assertLess(OP.LOG_BYTES, 64 << 20)
        for index in range(len(locations)):
            changed = copy.deepcopy(TREE)
            altered = validate_log_read_limits(changed)[index]
            ...
            with self.subTest(location=index), self.assertRaises(ValueError):
                validate_log_read_limits(changed)

    def test_full_launch_validation_and_absent_build_proofs_precede_mutation(self):
        calls = [node for node in memory_walk(TREE) if isinstance(node, ast.Call)]
        launch = [node for node in calls if isinstance(node.func, ast.Name) and node.func.id == 'validate_full_launch']
        evidence = [node for node in calls if isinstance(node.func, ast.Name) and node.func.id == 'validate_full_evidence']
        summary = [node for node in calls if isinstance(node.func, ast.Name) and node.func.id == 'full_log_summary']
        ...
        self.assertEqual((len(launch), len(evidence), len(summary), len(validate), len(constructors)), (1, 1, 1, 1, 1))
        self.assertLess(launch[0].lineno, validate[0].lineno)
        self.assertLess(validate[0].lineno, constructors[0].lineno)
        self.assertLess(summary[0].lineno, evidence[0].lineno)
        ...
        self.assertEqual(ast.dump(evidence[0].args[3]), ast.dump(EXPECTED_ABSENT_BINARY.operand))
        repeated = [node for node in calls if isinstance(node.func, ast.Attribute) and node.func.attr == 'require' and
                    node.args and ast.dump(node.args[0]) == ast.dump(EXPECTED_ABSENT_BINARY)]
...
def main():
    ...
    if sys.platform != 'linux' or os.geteuid() != 0 or not os.environ.get('SSH_CONNECTION') or len(sys.argv) != 2:
        raise SystemExit('Run only through authorized root SSH with the source49 full operator path.')
    path = Path(sys.argv[1])
    if path.name != 'dispose-source49-binding-full-failed-pair.py':
        raise SystemExit('Only the exact source49 full disposal operator is supported.')
    raw = path.read_bytes()
    TREE = ast.parse(raw, filename='<reviewed-source49-full-operator>')
    values = {}
    ...
    selected = {'fingerprint', 'exact_pair', 'validate_receipt', 'validate_failed_report', 'validate_terminal',
        'validate_full_launch', 'full_log_summary', 'validate_full_evidence',
    ...
    suite.run(result)
    print(json.dumps({'suite': 'source49-disposal-guards', 'status': 'passed' if result.wasSuccessful() else 'failed',
