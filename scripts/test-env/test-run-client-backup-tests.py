...
class RunnerGuardTests(unittest.TestCase):
    ...
    def test_arguments_only_accept_explicit_supported_schema_versions(self):
        for schema in (24, 25):
            ...
            RUNNER.validate_arguments(self.args)
        for schema in (None, True, "25", 23, 26, 25.0):
            self.args.schema = schema
            self.reject(lambda: RUNNER.validate_arguments(self.args))

    def test_catalog_arguments_require_schema25_without_test_selectors(self):
    ...
    def test_cli_defaults_to_schema24_and_requires_schema25_for_catalog_mode(self):
        self.replace(RUNNER.argparse, "_", lambda message: message)
        initialize = RUNNER.argparse.HelpFormatter.__init__
        def fixed_width(formatter, *args, **kwargs):
            initialize(formatter, *args, **dict(kwargs, width=80))
        ...
        self.assertEqual(RUNNER.main(base + ["--mode", "catalog", "--schema", "25"]), 0)
        self.assertEqual((factory.call_args.args[0].schema, factory.call_args.args[0].mode), (25, "catalog"))
    ...
    def change_catalog(self, version):
    ...
    def source_fixture(self, number="07", schema=24, bootstrap=False):
        ...
        for version in range(1, schema + 1):
            name = {24: RUNNER.MIGRATION_24_NAME, 25: RUNNER.MIGRATION_25_NAME}.get(version, f"{version:04d}_fixture.sql")
        ...
        for version in range(23, schema + 1):
            ...
            tables = [{"Name": f"fixture_{index:02d}", "Columns": ["id"], "PrimaryKey": ["id"], "SortKey": ["id"]}
                      for index in range(29 if version == 23 else 30)]
            tables[-1]["PrimaryKey"] = []
            if version == 25:
        ...
        if bootstrap:
            self.bootstrap_catalog_raw = self.source_bytes.pop(source / RUNNER.catalog_name(25))
    ...
    def generated_artifact_fixture(self):
        self.source_fixture(schema=25, bootstrap=True)
        self.args.schema, self.args.mode = 25, "catalog"
        runner = RUNNER.Runner.__new__(RUNNER.Runner)
        ...
        self.replace(RUNNER, "private_read", lambda path, **kwargs: runner.receipt_bytes if path == RUNNER.RECEIPT else self.bootstrap_catalog_raw)
        return runner
