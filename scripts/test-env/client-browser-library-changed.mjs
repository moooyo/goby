...
import { createLibraryChangedBrowserActor, authorityForURL,
...
import { loadLibraryChangedSource44Fixture } from './client-library-changed-source44-fixture.mjs';

const WORK = '/opt/goby-test/exec-work-m3e';
export const CHANGED_ROOT = WORK + '/client-library-changed-ui-source44-v1';
export const CHANGED_OUTPUT = CHANGED_ROOT + '/browser';
export const CHANGED_UNIT = 'goby-client-library-changed-ui-source44-v1.service';
export const CHANGED_CONTROLLER_UNIT = 'goby-client-library-changed-ui-source44-controller-v1.service';
...
const SOURCES = ['client-browser-library-changed.mjs', 'client-library-changed-source44-fixture.mjs',
...
function need(value, code = 'library_changed_guard_rejected') { if (!value) throw new Error(code); }
...
export function validateLibraryChangedInput(input) {
  ...
  need(exact(candidate, ['binary_sha256', 'process', 'runtime_sha256', 'state_sha256', 'server_id',
    'base_url', 'direct_url', 'source', 'source_manifest_sha256']) &&
    candidate.binary_sha256 === 'cd67f2e71ff1b63e3c138cdba1f9c9d1e584e2788b47964a332b38311cda0e2d' &&
    same(candidate.process, { pid: 1264063, start_ticks: 11104222, boot_id: '6bdfc486-7bc8-412f-82b5-70095a09dde7' }) &&
    candidate.runtime_sha256 === 'd8689a4e0b36816ed462816856dfa73af6fba5f31f044632ed173db99c8842df' &&
    candidate.state_sha256 === 'd6b8872eab70848b834c50e26cc2b84e9d137dd68049a1be85d09afa811829d4' &&
    candidate.server_id === SERVER && candidate.base_url === ORIGIN && candidate.direct_url === DIRECT &&
    candidate.source === WORK + '/source-attempt-44' &&
    candidate.source_manifest_sha256 === 'c2c9492589360b058c6533d0719219cf85ab5bc056bd76290cee51e7dc897f8b');
  ...
  need(exact(input.authority, ['continuation_report', 'continuation_terminal', 'current_snapshot', 'before_snapshot']) &&
    Object.values(input.authority).every(descriptor) && input.authority.before_snapshot.path === ROOT + '/before-full.json' &&
    input.authority.current_snapshot.path === WORK + '/client-notifications-source44-continuation-v1/after-full.json' &&
    input.authority.current_snapshot.sha256 === '738eb9717506bec997456830e70d56f0867f9cf8159939ea3733e039cadb9811');
  ...
  need(same(scripts.sort(), SOURCES.map(name => WORK + '/client-library-changed-source44-tool-02/' + name).sort()));
  return input;
}
...
export function validateLibraryChangedBaseline(input, before, current) {
  ...
  need(record(first) && record(previous) && before.schema === 27 && current.schema === 27);
  const metadata = value => { const result = clone(value); instant(result.captured_at); delete result.captured_at; return result; };
  need(same(first.tables, previous.tables) && same(first.sequences, previous.sequences) && same(first.catalog, previous.catalog) &&
    same(metadata(first.metadata), metadata(previous.metadata)) && instant(first.metadata.captured_at) >= instant(previous.metadata.captured_at));
  const tables = first.tables;
  need(Object.keys(tables).length === 35 && tables.sessions.length === 73 && tables.devices.length === 62 &&
    tables.activity_entries.length === 163 && tables.play_sessions.length === 26 && tables.user_item_data.length === 7 &&
...
export function pairLibraryChangedReads(physical, frames) {
  ...
  const fits = (frame, transfer) => transfer.completed && transfer.terminal_status === 200 && transfer.status === 200 &&
...
export function libraryChangedWindowEvidence(window, input, reservation) {
  ...
  for (const sample of window.dom) {
    need(inWindow(sample) && Number.isFinite(sample.started_elapsed_ms) && sample.started_elapsed_ms <= sample.elapsed_ms &&
      ...
      sample.observation.target_id === ITEM && sample.observation.expected_name === expected && sample.observation.media_inactive === true,
  ...
  const accepted = pairs.filter(pair => pair.complete && pair.frame.request_sequence > received.sequence &&
    ...
    pair.physical.projection.target.Id === ITEM && pair.physical.projection.target.Name === expected);
  ...
  const targetDOM = sample => sample.observation.passed === true && sample.observation.identity_proven === true &&
    sample.observation.visible_target_cards === 1 && sample.observation.target_title_count >= 1 && sample.observation.forbidden_title_count === 0;
...
export function encodeLibraryChangedRecord(value) { return Buffer.from(JSON.stringify(value, null, 2) + '\n'); }
...
export class LibraryChangedObserver {
  ...
  scope(raw, method) {
    ...
    if (selected && this.discovery && selected.kind !== 'target') {
      need(this.discovery.query_allowlist.some(item => item.shape_sha256 === selected.shape_sha256), 'library_changed_query_scope_changed');
    }
    return selected;
  }
...
export async function observeLibraryChangedDOM(page, target, expectedName, forbiddenName, documentID) {
  const route = libraryChangedRoute(page.url());
  const nodes = page.locator(`[data-id="${target.id}"]`);
  need(await nodes.count() <= 64, 'library_changed_dom_limit');
  const observed = await nodes.evaluateAll((elements, values) => {
    const visible = element => {
      ...
      return bounds.width > 0 && bounds.height > 0 && bounds.bottom > 0 && bounds.right > 0 &&
        bounds.top < innerHeight && bounds.left < innerWidth && style.display !== 'none' && style.visibility !== 'hidden';
    };
    ...
    const cards = roots.map(element => ({ tag: element.tagName.toLowerCase(), classes: String(element.className).slice(0, 256),
      id: element.getAttribute('data-id'), type: element.getAttribute('data-type') }));
    let targetTitle = 0, forbiddenTitle = 0, count = 0, overflow = false;
    if (roots.length === 1) {
      const walker = document.createTreeWalker(roots[0], NodeFilter.SHOW_ELEMENT);
      let element = roots[0];
      while (element) {
        if (++count > 512) { overflow = true; break; }
        if (visible(element) && element.closest('[data-id]')?.getAttribute('data-id') === values.id) {
          const ownText = [];
          for (const child of element.childNodes) {
            if (child.nodeType === 3 && child.nodeValue?.trim()) {
              if (ownText.length >= 64 || child.nodeValue.length > 4096) { overflow = true; break; }
              ownText.push(child);
            }
          }
          const text = ownText.map(node => node.nodeValue).join(' ').replace(/\s+/g, ' ').trim();
          const textVisible = ownText.length > 0 && ownText.every(node => {
            const range = document.createRange(); range.selectNodeContents(node);
            const box = range.getBoundingClientRect(); range.detach();
            return box.width > 0 && box.height > 0 && box.bottom > 0 && box.right > 0 && box.top < innerHeight && box.left < innerWidth;
          });
          if (textVisible && text.length <= 4096) {
            if (text === values.expected) targetTitle++;
            if (values.forbidden !== null && text === values.forbidden) forbiddenTitle++;
          }
        }
        element = walker.nextNode();
      }
    }
    return { cards, targetTitle, forbiddenTitle, overflow };
  }, { id: target.id, expected: expectedName, forbidden: forbiddenName });
  need(!observed.overflow && observed.targetTitle <= 64 && observed.forbiddenTitle <= 64, 'library_changed_dom_limit');
  const { cards, targetTitle, forbiddenTitle } = observed;
  ...
  const identity = cards.length === 1 && cards[0].id === target.id && (cards[0].type === null || cards[0].type === 'Movie');
  return { route, document_id: documentID, target_id: target.id, expected_name: expectedName,
    visible_target_cards: cards.length, target_title_count: targetTitle, forbidden_title_count: forbiddenTitle,
    identity_proven: identity, media_inactive: !active, passed: identity && targetTitle >= 1 && forbiddenTitle === 0 && !active,
    selector: { identity_attribute: 'data-id', title_mode: 'exact-visible-text', card_tag: cards.length === 1 ? cards[0].tag : null,
...
export function libraryChangedStageRecord(name, observation, binding, session, previousControl) {
  const shapes = { discovery: ['home', 'dom', 'reads', 'query_allowlist', 'socket'], armed: ['quiet', 'boundary'],
...
export class LibraryChangedWorkflow {
  ...
  async capture(expected, forbidden) {
    ...
    return bounded(this.dom(this.actor.page, this.input.target, expected, forbidden, this.observer.documentID), 3000);
  }
  ...
  async discovery() {
    ...
    do {
      ...
      if (dom.passed && dom.route !== previousRoute && !dom.route.includes(ITEM) && reads.some(pair => pair.complete && pair.physical.kind === 'items' &&
        ...
        pair.physical.projection.target.Name === this.input.target.name && pair.frame.token_sha256 === this.observer.boundToken())) break;
      await this.wait(100);
    } while (this.now() < end);
    need(dom?.passed && dom.route !== previousRoute && !dom.route.includes(ITEM), 'library_changed_target_card_not_observed');
    reads = reads.filter(pair => pair.complete && pair.physical.kind === 'items' && pair.frame.token_sha256 === this.observer.boundToken() &&
      pair.frame.page_route === dom.route && pair.frame.document_id === dom.document_id &&
    ...
    const observation = { home, dom, reads, query_allowlist: [...queries.values()], socket: this.observer.socket() };
  ...
  async finishWindow(name, boundary, control) {
    ...
    Object.assign(observation, libraryChangedWindowEvidence(observation, this.input, this.state.reservation));
...
export function libraryChangedObservationPassed(report) {
  try {
    ...
    for (const name of ['forward', 'restored']) need(libraryChangedWindowEvidence(report[name], { target: report.target }, reservation).result === 'passed');
    return true;
  } catch { return false; }
}
...
export async function runLibraryChanged(options) {
  process.umask(0o077);
  const parsed = parseLibraryChangedArguments(Object.entries(options).flatMap(([key, value]) => ['--' + key, value]));
  const input = validateLibraryChangedInput(await checkedHomeFile(parsed.input, parsed['input-sha256']));
  const rootIdentity = await privateDirectory(ROOT), node = await libraryChangedProcessIdentity(input);
  ...
  const baseline = validateLibraryChangedBaseline(input, before, current);
  ...
  const account = validateLibraryChangedViewer(credentials, input);
  const fixture = await loadLibraryChangedSource44Fixture({ input, inputSHA256: parsed['input-sha256'] });
  need(fixture.serverId === SERVER && typeof fixture.assertPinned === 'function'); await fixture.assertPinned();
  await fs.mkdir(OUTPUT, { mode: 0o700 }); const outputIdentity = await privateDirectory(OUTPUT);
  ...
  actor = createLibraryChangedBrowserActor({ account, pin, report: report.actor, observer: observer.hooks() }); observer.setActor(actor);
...
if (process.argv[1] && path.resolve(process.argv[1]) === SELF) {
  try {
    const report = await runLibraryChanged(parseLibraryChangedArguments(process.argv.slice(2)));
