#!/usr/bin/env python3
"""Observe one source44 native Name edit through an unchanged original client.
...
schema27 preservation ledgers and the independent browser worker lifetime.
...
TOOL = WORK / 'client-library-changed-source44-tool-02'
ROOT = WORK / 'client-library-changed-ui-source44-v1'
BROWSER_ROOT = ROOT / 'browser'
WORKER_UNIT = 'goby-client-library-changed-ui-source44-v1.service'
CONTROLLER_UNIT = 'goby-client-library-changed-ui-source44-controller-v1.service'
...
AUTHORITY = {
    'continuation_report': {'path': str(WORK / 'client-notifications-source44-continuation-v1/report.json'),
        'sha256': '4b426981fa6cc188ca0401c8b28005aeb871246390f1594ef66cd39ab550798e'},
    'continuation_terminal': {'path': str(WORK / 'client-notifications-source44-continuation-execution-01/terminal.json'),
        'sha256': '08d91a6611b5d2f1cac425528c60054235770fde57a4950414416dcc92a362b0'},
    'current_snapshot': {'path': str(WORK / 'client-notifications-source44-continuation-v1/after-full.json'),
        'sha256': '738eb9717506bec997456830e70d56f0867f9cf8159939ea3733e039cadb9811'},
}
FIXTURE = {
...
JS_NAMES = ('client-browser-library-changed.mjs', 'client-library-changed-source44-fixture.mjs',
...
MAX_JSON, MAX_IPC, MAX_REPORT = 32 << 20, 512 << 10, 4 << 20
...
def quiescent(snapshot):
    ...
    require(type(snapshot['schema']) is int and snapshot['schema'] == 27 and len(tables) == 35 and not tables['client_playback_references'] and not tables['encoding_jobs'],
            'The candidate lacks its idle complete schema27 profile.')
...
def reserve_edit(original, profile):
    ...
    marker = original['Effective']['Name'] + ' [LC source44 v1]'
...
def owned_metadata_audit(snapshot, session_id, number):
    ...
    return len(rows) == 1 and type(rows[0].get('revision')) is int and type(rows[0].get('affected_count')) is int and all(same(rows[0].get(key), value) for key, value in {
        ...
        'request_id': '', 'affected_count': 0, 'state': '', 'changed_fields': ['Name', 'Overrides']}.items())
...
def validate_ledger(before, after, profile, reservation=None, browser=None, administrator=None,
    ...
    for name, rows in left['tables'].items():
        ...
        require(isinstance(rows, list) and isinstance(current, list) and
                ...
                'A complete table row differs from its exact source44 column inventory: ' + name)
    ...
    require(all(type(row.get(key)) is int for row in audits for key in ('id', 'revision', 'affected_count')) and
    ...
    require(Counter(tuple(row[key] for key in keys) + (tuple(row['changed_fields']),) for row in audits) == Counter(expected_audits) and
            all(row['severity'] == 'Info' and row['actor_kind'] == 'user' and row['state'] == row['request_id'] == '' and
...
def catalog_shape(value):
    require(isinstance(value, dict) and value.get('kind') in ('items', 'target') and isinstance(value.get('query'), list) and
    ...
    require(query.get('userid', B) == B and query.get('ids', ITEM) == ITEM and
        ('limit' not in query or re.fullmatch(r'(?:[1-9][0-9]?|1[01][0-9]|12[0-8])', query['limit'])) and
        (value['kind'] == 'target' and value.get('route') in tuple(route + '/' + ITEM for route in routes) or
...
def pair_reads(physical, frames):
    ...
    for transfer in physical:
        ...
        if projection is not None:
            require(isinstance(projection, dict) and set(projection) == {'target', 'count', 'body_sha256', 'body_bytes'} and
                ...
                isinstance(projection['target'], dict) and set(projection['target']) == {'Id', 'Name', 'Type'} and
                projection['target']['Id'] == ITEM and projection['target']['Type'] == 'Movie' and safe_text(projection['target']['Name']),
...
def window_evidence(window, input_record, reservation):
    ...
    accepted = [pair for pair in pairs if pair['complete'] and pair['frame'].get('request_sequence', -1) > received['sequence'] and
...
def validate_stage(value, input_record, input_sha, child, name, previous, proof):
    ...
    keys = {'discovery': {'home', 'dom', 'reads', 'query_allowlist', 'socket'}, 'armed': {'quiet', 'boundary'},
...
class Run:
    ...
    def check(self):
        ...
        require(self.op.verify_service(self.state) == PROCESS, 'The running source44 candidate changed.')
        props = self.properties('goby-client-m3e.service')
        require(props.get('InvocationID') == INVOCATION and props.get('ActiveState') == 'active' and props.get('SubState') == 'running',
                'The source44 candidate invocation changed.')
    ...
    def snapshot(self, name=None):
        ...
        self.profile_reader.validate_structure(self.op, value, self.state)
        quiescent(value)
        return value
    ...
    def load(self):
        ...
        require(Path(__file__).absolute() == TOOL / 'observe-client-library-changed.py', 'The controller is outside its new frozen tool directory.')
        ...
        require(self.state.get('marker') == 'goby-m3e-client-acceptance-v1' and self.state.get('work') == str(WORK) and
            type(self.state.get('schema')) is int and self.state['schema'] == 27 and self.state.get('phase') == 'ready' and
            self.state.get('stage') == 'complete' and same(self.state.get('process'), PROCESS) and self.state.get('binary_sha256') == BINARY_SHA and
            self.state.get('runtime_sha256') == RUNTIME_SHA and self.state.get('server_id') == SERVER and
            self.state.get('browser_sha256') == CREDENTIALS_SHA and self.state.get('viewer_id') == B and self.state.get('admin_id') == ADMIN and
            self.state.get('notifications_upgrade', {}).get('phase') == 'complete', 'The exact source44 state is not a completed candidate authority.')
        self.artifacts = {str(self.args.source_closure): self.args.source_closure_sha256, str(CREDENTIALS): CREDENTIALS_SHA,
            **{value['path']: value['sha256'] for value in AUTHORITY.values()}, **{str(path): digest for path, digest in FIXTURE.values()}}
        report, terminal = read_record(AUTHORITY['continuation_report']), read_record(AUTHORITY['continuation_terminal'])
        require(report.get('status') == 'passed' and report.get('state_sha256') == STATE_SHA and report.get('source_manifest_sha256') == MANIFEST_SHA and
            report.get('publication') == PUBLICATION and report.get('binary', {}).get('sha256') == BINARY_SHA and same(report.get('new_process'), PROCESS) and
            all(report.get(key) is True for key in ('complete_rows_sequences_catalog_preserved', 'credentials_recovery_and_three_media_groups_preserved',
                'primary_identity_unchanged', 'original_failure_trees_preserved')) and
            all(report.get(key) is False for key in ('web_assets_replaced', 'schema_migration_performed', 'client_acceptance', 'automatic_retry', 'automatic_rollback')) and
            same(report.get('evidence', {}).get('after-full.json'), AUTHORITY['current_snapshot']), 'The completed continuation receipt changed scope.')
        require(terminal.get('status') == 'passed' and terminal.get('state_sha256') == STATE_SHA and
            terminal.get('report_sha256') == AUTHORITY['continuation_report']['sha256'] and terminal.get('recursive_cgroup_empty') is True and
            terminal.get('state', {}).get('MainPID') == '0' and terminal['state'].get('Result') == 'success' and
            terminal['state'].get('ExecMainStatus') == '0' and terminal.get('candidate_service', {}).get('InvocationID') == INVOCATION and
            terminal.get('primary_service', {}).get('InvocationID') == PRIMARY_INVOCATION, 'The completed source44 terminal is not an independent success.')
        ...
        compare_fixed_snapshot(read_record(AUTHORITY['current_snapshot']), self.before)
    ...
    def prepare(self):
        ...
        self.save('intent.json', {'marker': MARKER, 'version': 1, 'mode': MODE, 'root': str(ROOT), 'controller': controller,
            'controller_invocation': props['InvocationID'], 'worker_unit': WORKER_UNIT, 'sources': self.sources, 'authority': AUTHORITY,
            'native_maximum': 12, 'metadata_put_maximum': 2, 'before_authority': AUTHORITY['current_snapshot'],
        ...
        self.input = {'marker': INPUT_MARKER, 'version': 1, 'mode': MODE, 'root': str(ROOT), 'output': str(BROWSER_ROOT),
            ...
            'candidate': {'binary_sha256': BINARY_SHA, 'process': PROCESS, 'runtime_sha256': RUNTIME_SHA, 'state_sha256': STATE_SHA,
                'server_id': SERVER, 'base_url': BASE_URL, 'direct_url': DIRECT_URL, 'source': str(SOURCE), 'source_manifest_sha256': MANIFEST_SHA},
            ...
            'target': self.profile['target'], 'source_closure': self.js_sources, 'authority': {**AUTHORITY, 'before_snapshot': before}, 'controller': controller}
    ...
    def native(self, label, body=None, expected=200):
        ...
        self.save('native-' + label + '-intent.json', {'label': label, 'method': method, 'path': path,
            ...
            'session_id': self.administrator['id'] if self.administrator else None, 'candidate_process': PROCESS,
    ...
    def own_administrator(self, current):
        ...
        require(len(login_audits) == 1 and all(same(login_audits[0].get(key), value) for key, value in {
            ...
            'state': '', 'changed_fields': []}.items()) and type(login_audits[0]['revision']) is int and
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.profile_reader.validate_structure(self.op, self.after, self.state)
    ...
    def report(self):
        ...
        value = {'marker': MARKER, 'version': 1, 'mode': MODE, 'status': 'passed' if success else 'failed', 'phase': self.phase,
            ...
            'candidate_process': PROCESS, 'candidate_invocation': INVOCATION, 'state_sha256': STATE_SHA,
            'input_sha256': self.input_sha, 'source_closure_sha256': sha(canonical(self.js_sources)), 'authority': AUTHORITY,
        ...
        try:
            ...
            if self.args.check_only:
                return {'marker': MARKER, 'status': 'preflight_passed', 'http_requests': 0, 'evidence_writes': 0, 'service_writes': 0,
                    'candidate_process': PROCESS, 'state_sha256': STATE_SHA, 'target': self.profile['target'],
                    'current_authority': AUTHORITY['current_snapshot'], 'library_changed_client_acceptance': False}
            self.prepare()
            try:
                self.execute()
            except Exception as error:
...
def arguments(argv=None):
    ...
    require(all(HASH.fullmatch(value) for value in (args.script_sha256, args.source_closure_sha256, args.node_sha256)) and
