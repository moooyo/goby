...
func adminSettingsHTTPAssertSnapshot(t *testing.T, value map[string]any, revision string) {
	t.Helper()
	fields := []string{"Revision", "Defaults", "Overrides", "Effective", "Sources", "UpdatedAt", "Deployment"}
...
func TestHTTPAdminSettingsDefaultsExplicitOverridesNoopAndSelectiveReset(t *testing.T) {
	...
	if !reflect.DeepEqual(defaults, initial["Effective"]) {
	...
	if !reflect.DeepEqual(saved["Defaults"], defaults) || !reflect.DeepEqual(saved["Overrides"], defaults) || !reflect.DeepEqual(saved["Effective"], defaults) {
	...
	if objectValue(t, partial, "Effective")["ServerName"] != defaults["ServerName"] || objectValue(t, partial, "Sources")["ServerName"] != "deployment" ||
...
func TestHTTPAdminSettingsNativeAuthorityCSRFAndDeploymentSecretBoundary(t *testing.T) {
	...
	for _, operation := range []struct {
		...
		{http.MethodPut, "/admin/v1/settings", adminSettingsHTTPUpdate("1", "Denied", nil)},
		{http.MethodPost, "/admin/v1/settings/reset", map[string]any{"Revision": "1", "Fields": []string{"ServerName"}}},
	} {
	...
	if len(deployment) != 7 || strings.Contains(response.Body.String(), "private-marker") {
		t.Fatal("the startup-only deployment projection exposed configuration secrets or paths")
	}
	for _, field := range []string{"TranscodingEnabled", "HardwareDecoder", "HardwareEncoder", "Threads", "MaxJobs", "MaxUserJobs", "MaxSessionJobs"} {
...
func TestHTTPAdminSettingsConcurrentRevisionConflictReturnsCommittedWinner(t *testing.T) {
	...
	for _, name := range []string{"First proposed name", "Second proposed name"} {
		body, err := json.Marshal(adminSettingsHTTPUpdate("1", name, 8_000_000))
		if err != nil {
			t.Fatal(err)
		}
	...
	if winner == nil || conflicts != 1 || f.app.settings.Snapshot().Revision != 2 {
	...
	before := adminSettingsHTTPRow(t, f)
	stale := adminSettingsHTTPWrite(t, f, cookie, csrf, http.MethodPost, "/admin/v1/settings/reset", map[string]any{"Revision": "1", "Fields": []string{"ServerName"}})
...
func TestHTTPAdminSettingsRequestSnapshotRemainsConsistentAcrossConcurrentCommit(t *testing.T) {
	...
	if old.Revision != 1 || old.Effective != f.app.settings.Snapshot().Effective {
	...
	updated := adminSettingsHTTPObject(t, adminSettingsHTTPWrite(t, f, cookie, csrf, http.MethodPut, "/admin/v1/settings",
		adminSettingsHTTPUpdate("1", "A later committed server", 7_654_321)), http.StatusOK)
	adminSettingsHTTPAssertSnapshot(t, updated, "2")
	if objectValue(t, updated, "Effective")["ServerName"] != "A later committed server" ||
		objectValue(t, updated, "Effective")["MaxBitrate"] != float64(7_654_321) {
	...
	if fresh.Revision != 2 || fresh.Effective.ServerName != "A later committed server" || fresh.Effective.MaxBitrate != 7_654_321 ||
		planning.MaxBitrate != fresh.Effective.MaxBitrate || planning.MaxWidth != fresh.Effective.MaxWidth ||
...
func TestHTTPAdminSettingsExpiredActorWaitingForBusinessRowCannotCommit(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodPut, "/admin/v1/settings", strings.NewReader(adminSettingsUpdateBodyForTest("ServerName", `"Must not commit"`))).WithContext(f.ctx)
	request.AddCookie(cookie)
	request.Header.Set("Content-Type", "application/json")
	request.Header.Set("X-CSRF-Token", csrf)
...
func TestHTTPAdminSettingsTransactionAudienceAndStoredFailureDoNotLeak(t *testing.T) {
	...
	if _, err := f.pool.Exec(f.ctx, "UPDATE managed_settings SET server_name = $1 WHERE id = 1", secret); err != nil {
