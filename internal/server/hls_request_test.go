...
func TestHLSRequestRejectsInvalidOrUnsupportedExplicitConstraints(t *testing.T) {
	for _, test := range []struct {
		...
		{"missing-subtitle", map[string]string{"SubtitleStreamIndex": "12"}, errHLSRequestInvalid},
		...
		{"unsupported-video", map[string]string{"VideoCodec": "vp9"}, errHLSRequestUnsupported},
		...
		{"conflicting-container", map[string]string{"Container": "mp4", "SegmentContainer": "ts"}, errHLSRequestInvalid},
		...
		{"HDR", map[string]string{"VideoRangeType": "DolbyVision"}, errHLSRequestUnsupported},
		{"copy-timestamps", map[string]string{"CopyTimestamps": "true"}, errHLSRequestUnsupported},
		{"invalid-adaptive-flag", map[string]string{"EnableAdaptiveBitrate": "perhaps"}, errHLSRequestInvalid},
