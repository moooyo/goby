...
func (s *Store) NextUp(ctx context.Context, query NextUpQuery) (ItemResult, error) {
	...
	if query.SeriesID != "" {
		...
		err := tx.QueryRow(ctx, `SELECT i.id FROM items i WHERE i.id = $1 AND i.type = 'Series' AND i.is_folder
			AND ($2::boolean OR i.library_id = ANY($3::text[])) AND `+access.ordinarySQL("i"), query.SeriesID, access.all, access.folders).Scan(&id)
...
func nextUpScopeSQL(scopes ...libraryAccess) string {
	...
	return `WITH RECURSIVE parent_scope AS (
		SELECT i.id, i.library_id FROM items i WHERE i.id = $5::text
			AND ($2::boolean OR i.library_id = ANY($3::text[])) AND ` + access.ordinarySQL("i") + `
		UNION
		...
		FROM items series WHERE series.type = 'Series' AND series.is_folder
			...
			AND ($4::text = '' OR series.id = $4) AND ` + access.ordinarySQL("series") + `
		UNION
		...
		WHERE child.type <> 'Series' AND ` + access.ordinarySQL("child") + `
	), episode_state AS (
		...
		WHERE episode.type = 'Episode' AND NOT episode.is_folder AND ` + access.ordinarySQL("episode") + `
	), ordered_episodes AS (
