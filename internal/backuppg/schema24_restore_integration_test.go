...
// offline retry must preserve those rows while applying the real schema26 DDL.
func TestPostgreSQLOfflineSchema24RestorePreservesDataAndRetriesFinalizer(t *testing.T) {
	...
	failed, restoreErr := RestoreOfflineFinalized(ctx, target, archive, facts, offlineOptions,
		func(callbackCtx context.Context, tx pgx.Tx, result RestoreResult) error {
			if result.SourceVersion != 24 || result.CurrentVersion != 26 || !equalJSON(result.Tables, facts.Tables) {
	...
	if err != nil {
		...
		t.Fatalf("retry the real schema24 archive into schema26: %v", err)
	}
	if result.SourceVersion != 24 || result.CurrentVersion != 26 || !equalJSON(result.Tables, facts.Tables) {
		t.Fatal("schema24 retry changed source facts or did not reach schema26")
	}
	if version, err := database.SchemaVersion(ctx, target); err != nil || version != 26 {
		t.Fatalf("schema24 restored target version=%d, want 26: %v", version, err)
	}
	...
	var musicMigrationName, themeMigrationName string
	if err := target.QueryRow(ctx, `SELECT count(*),max(name) FILTER (WHERE version=25),max(name) FILTER (WHERE version=26)
		FROM schema_migrations`).Scan(&historyCount, &musicMigrationName, &themeMigrationName); err != nil || historyCount != 26 ||
		musicMigrationName != "0025_music_artists.sql" || themeMigrationName != "0026_theme_owners.sql" {
		t.Fatal("schema24 restoration did not append exactly the published music and theme migrations")
	}
}
