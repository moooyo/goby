...
// offline retry must preserve those rows while applying the real schema25 DDL.
func TestPostgreSQLOfflineSchema24RestorePreservesDataAndRetriesFinalizer(t *testing.T) {
	...
	failed, restoreErr := RestoreOfflineFinalized(ctx, target, archive, facts, offlineOptions,
		func(callbackCtx context.Context, tx pgx.Tx, result RestoreResult) error {
			if result.SourceVersion != 24 || result.CurrentVersion != 25 || !equalJSON(result.Tables, facts.Tables) {
	...
	if err != nil {
		...
		t.Fatalf("retry the real schema24 archive into schema25: %v", err)
	}
	if result.SourceVersion != 24 || result.CurrentVersion != 25 || !equalJSON(result.Tables, facts.Tables) {
		t.Fatal("schema24 retry changed source facts or did not reach schema25")
	}
	if version, err := database.SchemaVersion(ctx, target); err != nil || version != 25 {
		t.Fatalf("schema24 restored target version=%d, want 25: %v", version, err)
	}
	...
	if err := target.QueryRow(ctx, `SELECT count(*),max(name) FILTER (WHERE version=25) FROM schema_migrations`).Scan(&historyCount, &migrationName); err != nil || historyCount != 25 || migrationName != "0025_music_artists.sql" {
		t.Fatal("schema24 restoration did not append exactly the published schema25 migration")
	}
}
