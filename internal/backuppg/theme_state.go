...
func validateResourceState(ctx context.Context, tx pgx.Tx, version int64) error {
	...
	if err := validateSelectedPhase2State(ctx, tx, version); err != nil {
		return err
	}
	...
	if err := validateCompatibilityLongTailState(ctx, tx, version); err != nil {
		return err
	}
	return validateAnalysisState(ctx, tx, version)
}
