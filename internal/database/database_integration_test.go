...
func TestMigrateConcurrentAndIdempotent(t *testing.T) {
	...
	if err != nil || version != 10 {
		t.Fatalf("schema version after concurrent migration = %d, want 10, error = %v", version, err)
	}
	var count int
	...
	if version, err := database.SchemaVersion(ctx, pool); err != nil || version != 10 {
		t.Errorf("schema version after repeated migration = %d, want 10, error = %v", version, err)
	}
