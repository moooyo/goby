...
func TestMigrateConcurrentAndIdempotent(t *testing.T) {
	...
	if err != nil || version != 20 {
		t.Fatalf("schema version after concurrent migration = %d, want 20, error = %v", version, err)
	}
	var count int
	...
	if count != 20 {
		t.Fatalf("migration history count = %d, want 20", count)
	}
	...
	if version, err := database.SchemaVersion(ctx, pool); err != nil || version != 20 {
		t.Errorf("schema version after repeated migration = %d, want 20, error = %v", version, err)
	}
