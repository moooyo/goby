...
func TestMigrateConcurrentAndIdempotent(t *testing.T) {
	...
	if err != nil || version != 18 {
		t.Fatalf("schema version after concurrent migration = %d, want 18, error = %v", version, err)
	}
	var count int
	...
	if count != 18 {
		t.Fatalf("migration history count = %d, want 18", count)
	}
