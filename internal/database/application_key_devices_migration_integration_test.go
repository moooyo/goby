...
func applicationKeyDeviceLegacySnapshot(t *testing.T, ctx context.Context, pool *pgxpool.Pool) string {
	...
	if err := pool.QueryRow(ctx, `SELECT jsonb_build_object(
		'users', (SELECT jsonb_agg(to_jsonb(u) - 'configuration_revision'
		...
		'credentials', (SELECT jsonb_agg(to_jsonb(a) - 'device_registry_id' - 'local_auth' ORDER BY id) FROM sessions a),
...
func TestMigrateApplicationKeyDevicesPreservesSchema16CredentialsAndProjection(t *testing.T) {
	...
	if version, err := database.SchemaVersion(ctx, pool); err != nil || version != currentMigrationVersion(t) {
		t.Fatalf("application device schema = %d, want current, error=%v", version, err)
	}
