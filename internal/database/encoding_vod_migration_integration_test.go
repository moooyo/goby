...
func encodingVODDataSnapshot(t *testing.T, ctx context.Context, pool *pgxpool.Pool) string {
	...
	if err := pool.QueryRow(ctx, `SELECT jsonb_build_object(
		...
		'play', (SELECT jsonb_agg(to_jsonb(t) - 'client_correlated' ORDER BY id) FROM play_sessions t),
...
func TestMigrateEncodingVODPlansPreservesVersion10DataAndJSONBounds(t *testing.T) {
	...
	if version, err := database.SchemaVersion(ctx, pool); err != nil || version != 15 {
		t.Fatalf("upgraded schema version = %d, want 15, error = %v", version, err)
	}
	...
	if err := pool.QueryRow(ctx, "SELECT count(*) FROM schema_migrations").Scan(&count); err != nil || count != 15 {
		t.Fatalf("upgraded migration count = %d, want 15, error = %v", count, err)
	}
