...
func TestMigrateEncodingVODPlansPreservesVersion10DataAndJSONBounds(t *testing.T) {
	...
	if version, err := database.SchemaVersion(ctx, pool); err != nil || version != 20 {
		t.Fatalf("upgraded schema version = %d, want 20, error = %v", version, err)
	}
	...
	if err := pool.QueryRow(ctx, "SELECT count(*) FROM schema_migrations").Scan(&count); err != nil || count != 20 {
		t.Fatalf("upgraded migration count = %d, want 20, error = %v", count, err)
	}
