...
func TestEvaluateSubtitleDelivery(t *testing.T) {
	for _, test := range []struct {
		...
		{"embedded subtitle extraction", profileTestPtr(12), &SubtitleProfile{Format: "subrip", Method: SubtitleDeliveryMethodExternal}, true},
	} {
		t.Run(test.name, func(t *testing.T) {
			request := profileTestRequest()
			...
			if test.want {
				...
				if wantIndex == 12 && decision.SubtitleMethod != test.profile.Method {
					t.Fatalf("selected subtitle method = %q, want %q", decision.SubtitleMethod, test.profile.Method)
				}
			}
		})
	}
