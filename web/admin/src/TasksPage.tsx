...
export function TasksPage({ onLibraries, currentUserId, onNavigationGuardChange }: { onLibraries: () => void; currentUserId: string; onNavigationGuardChange: UserNavigationGuardChange }) {
  ...
  return <Box>
    <PageHeading title="Tasks" description="Run library scans, refresh metadata, download subtitles, maintain caches, and manage task schedules." action={<Button variant="outlined" startIcon={<LibraryBooksOutlined />} onClick={onLibraries}>Manage libraries</Button>} />
