...
export interface UserConfiguration {
  ...
  IntroSkipMode: 'None' | 'ShowButton' | 'AutoSkip'; EnableLocalPassword: boolean; ProfilePin?: string | null;
}
...
function decode(result: UserPreferences, id: string): UserPreferences {
  ...
  if (!result || result.UserId !== id || typeof result.Revision !== 'string' || !/^[1-9]\d*$/.test(result.Revision) || BigInt(result.Revision) > 9223372036854775807n || !config
    ...
    || (config.ProfilePin !== undefined && config.ProfilePin !== null && config.ProfilePin !== '')
    || !subtitleModes.includes(config.SubtitleMode) || !introSkipModes.includes(config.IntroSkipMode)
  ...
  return { ...result, Configuration: { ...config, ProfilePin: undefined } };
}
